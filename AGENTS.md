...
- Drafting ("Create post", "Regenerate") calls `generateSocialPostText` in `lib/summaries.js` through `composeSocialPostDraft` in `lib/socialPostDrafting.js`. The provider and model are `siteSettings.llmProvider || LLM_PROVIDER || 'openrouter'` and `siteSettings.llmModel || LLM_MODEL || DEFAULT_LLM_MODEL`, like the SEO refresh; the account name comes from `resolveSiteTitle`. The model writes a plain-language post of at most 240 characters faithful to the lay summary (no hype, advice, emojis, hashtags, handles or links) and the link is appended. OpenRouter requests use low reasoning effort (`SOCIAL_POST_REASONING`) with an 800-token content budget plus `REASONING_TOKEN_HEADROOM`. A draft too long for X is retried once with a 200-character limit; if that fails, or no credential is configured, the post uses the template (`xIntro`, title, link) and `generatedBy` is `template` instead of `llm:<model>`.
- Settings: `siteSettings.socialPosting.postToX` ("Offer new publications for X posts"; fail closed: off or unset makes dispatch return `skipped` and blocks drafting and queueing, while undo still works), `.xIntro` (template opening, default `New publication:`), and `.approverEmails` (falls back to `studyApprovals.admins`). The settings query excludes Studio drafts.
