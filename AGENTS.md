...
- If a user only mentions albuminuria/proteinuria qualitatively and a likely study depends on a urine protein threshold, the assistant may ask one focused follow-up for a recent quantitative value (`ACR`, `PCR`, or `24-hour urine protein`). If no number is available, do not loop on the same question; keep the study `possible` and let coordinator review confirm the threshold.
