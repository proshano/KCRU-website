...
- The admin approves every post before it reaches Buffer, so the first sync (no `socialPost` records yet) brings every current feed item to approvers as `pending`, and the `MAX_NEW_POSTS_PER_SYNC` (10) guard does not apply there (the feed itself is capped at 50 items). Once records exist, a sync that finds more than `MAX_NEW_POSTS_PER_SYNC` new items creates nothing and returns HTTP 500 so the workflow fails visibly. If those papers are expected, recover by posting `{ "seed": true }` to the dispatch route, which marks the current unrecorded items `seeded` without sending them for review.
