...
- Researchers with namesakes can use `publicationAuthorName` for a precise author name and `publicationExclusions` for verified false-positive PMIDs or DOIs. Apply exclusions per researcher so legitimate consortium papers attributed to other KCRU researchers remain available.
