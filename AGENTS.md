...
- When attribution review is enabled, `publicationAttributionReview` documents store deterministic researcher-plus-DOI/PMID decisions and normalized candidate snapshots. Researcher-specific PubMed results, exact name-compatible author ORCID, and the documented affiliation/recurring-coauthor thresholds publish automatically; ambiguous secondary-source links remain pending and outside public provenance. A recurring coauthor must occur on at least two PubMed-confirmed papers for that researcher.
