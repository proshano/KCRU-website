...
- Social post migration: `npm run migrate:social-posts` (dry run) and `npm run migrate:social-posts -- --apply` convert legacy `pending`/`skipped` social posts to `available`/`dismissed`.
