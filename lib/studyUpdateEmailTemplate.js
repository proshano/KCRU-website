...
export function buildStudyUpdateEmail({ subscriber, studies = [], manageUrl, monthLabel }) {
  const recipientName = sanitizeText(subscriber?.name)
  const recipientEmail = sanitizeText(subscriber?.email)
  const greeting = recipientName ? `Hi ${recipientName},` : 'Hello,'
  const title = monthLabel ? `Monthly study updates - ${monthLabel}` : 'Monthly study updates'
  const intro = studies.length
    ? 'Here are this month\'s studies that may be relevant to your patients.'
    : 'There are no recruiting studies to share right now.'
  ...
  const html = `
    <div style="font-family: system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif; font-size: 14px; color: #111; line-height: 1.5; background: #ffffff;">
      ...
      <p style="margin: 0; font-size: 12px; color: ${MUTED_TEXT};">London Kidney Clinical Research</p>
    </div>
  `
