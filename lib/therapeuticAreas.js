...
const THERAPEUTIC_AREA_WITH_STUDIES_QUERY = `
  *[_type == "therapeuticArea" && active == true] | order(order asc, name asc) {
    _id,
    name,
    shortLabel,
    "slug": slug.current,
    "studyCount": count(*[_type == "trialSummary" && references(^._id)])
  }
`
...
export async function fetchTherapeuticAreasWithStudies(fetcher = sanityFetch) {
  const rows = await fetcher(THERAPEUTIC_AREA_WITH_STUDIES_QUERY)
  const areas = Array.isArray(rows) ? rows : []
  return areas.filter((area) => Number(area?.studyCount || 0) > 0)
}
