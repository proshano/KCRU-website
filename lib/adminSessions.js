...
import { verifyAdminPasscodeChallenge } from './adminPasscodeVerification.js'
import { sanitizeString } from '@/lib/studySubmissions'
import { claimSecurityRateLimit } from '@/lib/securityRateLimit'

const ADMIN_SESSION_TYPE = 'adminSession'
const ADMIN_CREDENTIAL_TYPE = 'adminCredential'
const ADMIN_SCOPES = {
  approvals: 'approvals',
  updates: 'updates',
  any: 'any',
...
export async function verifyAdminPasscode({ email, code, sessionTtlHours }) {
  ...
  const session = await verifyAdminPasscodeChallenge({
    client: writeClient,
    email: normalized,
    code: cleanedCode,
    sessionTtlHours,
  })
  return { token: session.token, email: normalized }
}
...
function parsePasswordHash(value) {
  ...
  const [prefix, digest, iterations, salt, hash] = parts
  if (prefix !== PASSWORD_HASH_PREFIX) return null
  const parsedIterations = Number(iterations)
  if (!Number.isFinite(parsedIterations) || parsedIterations <= 0) return null
  if (!salt || !hash || digest !== PASSWORD_HASH_DIGEST) return null
  return { iterations: parsedIterations, salt, hash, digest }
}
...
async function getAdminCredential(email) {
  return writeClient.fetch(
    `*[_type == "${ADMIN_CREDENTIAL_TYPE}" && email == $email && disabled != true] | order(_updatedAt desc)[0]{
      _id,
      email,
      passwordHash,
      disabled
...
export async function verifyAdminPassword({ email, password, sessionTtlHours }) {
  const normalized = sanitizeString(email).toLowerCase()
  const cleanedPassword = sanitizeString(password)
  if (!normalized || !cleanedPassword) {
    throw new Error('Email and password are required.')
  }

  if (!writeClient.config().token) {
    throw new Error('SANITY_API_TOKEN missing; cannot verify password.')
  }

  await claimSecurityRateLimit({
    namespace: 'admin-password-verify',
    key: normalized,
    limit: 10,
    windowMs: 15 * 60 * 1000,
  ...
  if (!credential?._id || !credential.passwordHash) {
    throw new Error('Password not set. Request a passcode.')
  }
  ...
  const valid = verifyPasswordHash(cleanedPassword, credential.passwordHash)
  if (!valid) {
    throw new Error('Invalid password.')
  }

  const session = await createAdminTokenSession({
    email: normalized,
    sessionTtlHours,
  })

  return { token: session.token, email: normalized, expiresAt: session.expiresAt }
}
