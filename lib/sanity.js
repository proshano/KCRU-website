...
import { RESEARCH_DIGEST_MAX_PAPERS_CEILING } from './researchDigestConfig.js'
...
export const queries = {
  ...
  siteSettings: `
    *[_type == "siteSettings"][0] {
      ...
      publicationClassification{
        ...
        backendUpdatedBy,
        jevThresholdTopics,
        jevThresholdStudyDesign,
        jevThresholdMethodologicalFocus,
        jevThresholdExclude,
        thresholdsSource,
        thresholdsUpdatedAt
      },
  ...
  researchDigestIssues: `
    *[_type == "researchDigestIssue" && status in ["approved", "sent"]] | order(date desc)[0...20] {
      _id,
      title,
      date,
      "slug": slug.current,
      ...
      approvedAt,
      sentAt,
      "paperCount": count(*[_type == "researchDigestPaper" && issueDate == ^.date && approvalStatus == "approved" && autoSelectionExcluded != true][0...${RESEARCH_DIGEST_MAX_PAPERS_CEILING}])
    }
  `,

  researchDigestIssueBySlug: `
    *[_type == "researchDigestIssue" && slug.current == $slug && status in ["approved", "sent"]][0] {
      _id,
      title,
      date,
      "slug": slug.current,
      ...
      approvedAt,
      sentAt,
      "papers": *[_type == "researchDigestPaper" && issueDate == ^.date && approvalStatus == "approved" && autoSelectionExcluded != true] | order(priorityScore desc, tier asc, journal asc, title asc)[0...${RESEARCH_DIGEST_MAX_PAPERS_CEILING}] {
        _id,
        pmid,
        doi,
        title,
  ...
  approvedResearchOpportunities: `
    *[_type == "researchOpportunity" && approvalStatus == "approved" && status in ["open", "upcoming"] && (!defined(deadline) || deadline >= $today)] | order(deadline asc, title asc) {
      _id,
      type,
      status,
      sourceName,
