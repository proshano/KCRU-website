...
// Hard ceiling on how many papers a subscriber can ever receive in one day.
// Settings may lower this, never raise it.
export const RESEARCH_DIGEST_MAX_PAPERS_CEILING = 3

// How many days a qualifying-but-unselected paper may wait for a slot before it goes stale.
export const RESEARCH_DIGEST_MAX_CARRYOVER_DAYS = 30

export const RESEARCH_DIGEST_SELECTION_STATUS = Object.freeze({
  selected: 'selected',
  deferred: 'deferred',
...
export const DEFAULT_RESEARCH_DIGEST_SETTINGS = Object.freeze({
  publicEnabled: false,
  subjectTemplate: '{{leadTitle}}{{andMore}} - {{date}}',
  introText: 'Here is today’s most useful new kidney research for {{date}}.',
...
export function normalizeResearchDigestSettings(settings = {}) {
  ...
  const rawMinPriorityScore = settings?.minPriorityScore
  const minPriorityScore = rawMinPriorityScore === null || rawMinPriorityScore === undefined || rawMinPriorityScore === ''
    ? Number.NaN
    : Number(rawMinPriorityScore)
  const rawCarryoverDays = settings?.carryoverDays
  const carryoverDays = rawCarryoverDays === null || rawCarryoverDays === undefined || rawCarryoverDays === ''
    ? Number.NaN
    : Number(rawCarryoverDays)

  return {
    ...DEFAULT_RESEARCH_DIGEST_SETTINGS,
    ...settings,
    maxPapers: Number.isFinite(maxPapers) && maxPapers > 0
      ? Math.min(Math.round(maxPapers), RESEARCH_DIGEST_MAX_PAPERS_CEILING)
    ...
    minPriorityScore: Number.isFinite(minPriorityScore)
      ? Math.min(Math.max(Math.round(minPriorityScore), 0), 100)
      : DEFAULT_RESEARCH_DIGEST_SETTINGS.minPriorityScore,
    carryoverDays: Number.isFinite(carryoverDays)
      ? Math.min(Math.max(Math.round(carryoverDays), 0), RESEARCH_DIGEST_MAX_CARRYOVER_DAYS)
      : DEFAULT_RESEARCH_DIGEST_SETTINGS.carryoverDays,
    automaticSelection: settings?.automaticSelection !== false,
    publicEnabled: settings?.publicEnabled === true,
    sendEmpty: Boolean(settings?.sendEmpty),
    pilotMode: settings?.pilotMode === true,
