...
export async function fetchSocialPostPrompt(client) {
  const doc = await client.fetch(
    '*[_id == $id && !(_id in path("drafts.**"))][0] { systemPrompt, updatedBy, updatedAt, _rev }',
    { id: SOCIAL_POST_PROMPT_ID }
  )
  return {
...
// Creates the singleton doc on first use, then patches it. Guarded with ifRevisionId(rev)
...
// meanwhile.
async function writeSocialPostPrompt(writeClient, fields, rev) {
  const existing = await writeClient.getDocument(SOCIAL_POST_PROMPT_ID)
  await writeClient.createIfNotExists({ _id: SOCIAL_POST_PROMPT_ID, _type: SOCIAL_POST_PROMPT_TYPE })
  if (!rev && existing) {
  ...
  try {
    let patch = writeClient.patch(SOCIAL_POST_PROMPT_ID)
    if (rev) patch = patch.ifRevisionId(rev)
    return await applyFields(patch, fields).commit()
  } catch (error) {
    throw markConflict(error)
  }
}
...
export async function saveSocialPostPrompt(writeClient, { text, actorEmail, rev, now = new Date() } = {}) {
...
export async function resetSocialPostPrompt(writeClient, { actorEmail, rev, now = new Date() } = {}) {
  return writeSocialPostPrompt(writeClient, {
    systemPrompt: null,
    ...promptWriteFields(actorEmail, now),
  }, rev)
