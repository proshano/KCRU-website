...
import { getProvenanceIds, publicationAnchorFromKey } from './publicationIdentity.js'
import {
  DEFAULT_TEAM_LABEL,
  SOCIAL_NETWORK_X,
...
// The names are used to match the paper's first author, so the first-author rule can pick
// whose profile the post links to.
async function fetchTeamResearchers(client, ids = []) {
  if (!ids.length) return new Map()
  const researchers = await client.fetch(
    '*[_type == "researcher" && _id in $ids] { _id, name, publicationAuthorName, publicationAuthorAliases }',
    { ids }
  )
  return new Map((researchers || []).filter((researcher) => researcher?._id).map((researcher) => [researcher._id, researcher]))
}
...
export async function syncSocialPosts({
  ...
  // The same selection as /publications/feed.xml, so X and the RSS feed agree on which papers.
  ...
  const researchersById = await fetchTeamResearchers(client, researcherIds)
  const status = plan.mode === 'seed' ? 'seeded' : 'available'
  const built = toWrite.map((item) => {
    const researchers = getProvenanceIds(item.publication, provenance).map((id) => researchersById.get(id)).filter(Boolean)
    return buildSocialPostRecord({
      item,
      network: SOCIAL_NETWORK_X,
      teamMembers: researchers.map((researcher) => researcher.name).filter(Boolean),
      researchers,
      status,
      now,
    })
...
// The paper's investigators who have a profile page. Records from before teamMemberIds
// was stored fall back to matching the recorded team member names.
function findSpotlightCandidates(post = {}, researchers = []) {
  const ids = Array.isArray(post.teamMemberIds) ? post.teamMemberIds.filter(Boolean) : []
  let candidates
  if (ids.length) {
    const byId = new Map(researchers.map((researcher) => [researcher._id, researcher]))
    candidates = ids.map((id) => byId.get(id)).filter(Boolean)
  } else {
    const names = (post.teamMembers || []).map((name) => cleanString(name).toLowerCase()).filter(Boolean)
    candidates = names.flatMap((name) => researchers.filter((researcher) => cleanString(researcher.name).toLowerCase() === name))
  }
  return Array.from(new Map(candidates.map((researcher) => [researcher._id, researcher])).values())
}

// Hosts that only work on the machine running the site. getSiteBaseUrl() falls back to
// http://localhost:3000 when SITE_URL is unset, and such a link must never reach a post.
...
function isPublicHttpsUrl(value) {
  ...
  return url.protocol === 'https:' && !LOCAL_HOSTS.has(host) && !host.endsWith('.localhost') && !/^127\./.test(host)
}

// Chooses whose profile a draft links to (see chooseSpotlightResearcher) and builds the
// link, anchored to the paper. Regenerate keeps the profile already chosen while it is
// still one of the paper's investigators, unless the first author now gets the link.
// Returns { researcherId, name, slug, reason, link }, or null when no investigator on the
// paper has a profile or the site has no public https address, so the post links to the paper.
export async function resolveSocialPostSpotlight({
  client,
  post = {},
  regenerate = false,
  baseUrl,
  ...
  if (!isPublicHttpsUrl(baseUrl)) return null
  const [researchers, records] = await Promise.all([
    client.fetch('*[_type == "researcher" && defined(slug.current) && !(_id in path("drafts.**"))] { _id, name, "slug": slug.current }'),
    fetchSocialPostRecords(client, post.network || SOCIAL_NETWORK_X),
  ])
  const withProfile = (researchers || []).filter((researcher) => researcher?._id && cleanString(researcher.slug))
  const candidates = findSpotlightCandidates(post, withProfile)

  const chosen = chooseSpotlightResearcher({
    candidates,
    firstAuthorId: post.firstAuthorId,
    recentSpotlights: collectRecentSpotlights({ records, researchers: withProfile, now, excludeId: post._id }),
    random,
  })
  // firstAuthorId can be filled in after the draft was made (for example by the migration),
  // so on Regenerate the first author still wins over the earlier pick.
  const keptSlug = regenerate && chosen?.reason !== 'first-author' ? cleanString(post.spotlightSlug).toLowerCase() : ''
  const kept = keptSlug ? candidates.find((researcher) => cleanString(researcher.slug).toLowerCase() === keptSlug) : null
  const choice = kept ? { researcher: kept, reason: cleanString(post.spotlightReason) } : chosen
  if (!choice) return null

  const slug = cleanString(choice.researcher.slug)
  const anchor = cleanString(post.paperAnchor) || publicationAnchorFromKey(post.guid)
  return {
    researcherId: choice.researcher._id,
    name: cleanString(choice.researcher.name),
    slug,
    reason: choice.reason,
    link: buildProfilePaperLink({ baseUrl, slug, anchor }),
  }
}
