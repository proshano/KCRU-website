...
/**
 * Fetch a binary asset (a publisher's first-page image) through the headless browser.
 *
 * Used only after the plain fetch of the same URL was refused: the browser session can
 * pass bot protection that blocks a bare request. Every request the navigation makes is
 * validated against the public-network rules before it leaves, the response must carry an
 * allowed content type, and the body is bounded. Returns null on any failure.
 */
export async function browserFetchBytes(url, {
  ...
  try {
    ...
    if (!response.ok()) throw new Error(`Browser asset request failed (${response.status()}).`)

    const headers = response.headers() || {}
    const contentType = String(headers['content-type'] || '').toLowerCase()
    if (allowedContentTypes.length && !allowedContentTypes.some((type) => contentType.startsWith(type))) {
      throw new Error(`Browser asset content type is not allowed: ${contentType || '(missing)'}.`)
    }
    const contentLength = Number(headers['content-length'])
    if (Number.isFinite(contentLength) && contentLength > maxBytes) {
      throw new Error(`Browser asset exceeds the ${maxBytes}-byte limit.`)
    }

    const bytes = await response.buffer()
    if (!bytes?.byteLength) throw new Error('Browser asset was empty.')
    if (bytes.byteLength > maxBytes) throw new Error(`Browser asset exceeds the ${maxBytes}-byte limit.`)
    return { bytes, contentType, finalUrl: response.url() }
  } catch (err) {
    console.warn(`[browserFetch] Asset fetch failed for ${url} — ${err?.message || 'unknown'}`)
    return null
  } finally {
    if (page) {
      try { await page.close() } catch { /* ignore */ }
