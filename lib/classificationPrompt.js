...
First, think internally step-by-step through the classification using the following headings:
...
Return JSON only, with no reasoning or extra text.

Set \`"exclude": true\` for corrections or errata only.

-----

## Topic Categories (assign 0+)

Use an empty topics array when a publication is primarily about research methodology, trial design, reporting standards, estimands, or statistical frameworks and does not focus on a clinical disease area or healthcare delivery problem.
...
| **Health Systems** | Healthcare delivery, access, quality improvement, health policy, health services organization, or disparities in care delivery (excluding remote care/clinical decision support). **Do NOT use** for papers about clinical trial design, reporting standards, estimands, statistical methods, research frameworks, or guidance intended to improve how studies are planned, analyzed, or interpreted. |
...
## Study Design (assign 0+)

**Select ALL that apply.** For example, a study with both a survey and focus groups should be tagged as both "Observational Study" and "Qualitative Study". Return an empty array when none of the study-design categories fits.
...
| **Clinical Practice Guideline** | Recommendations for clinical care: diagnosis, treatment, monitoring, prevention, or management of patients. **Do NOT use** for research-methods guidance, trial-design frameworks, reporting standards, statistical frameworks, estimand guidance, ICH extensions, or consensus statements about how research should be designed, analyzed, or interpreted. If no other study-design tag fits, return an empty \`study_design\` array and use the relevant Methodological Focus tags. |
...
| **Commentary / Editorial** | Opinion, editorial, letter, perspective, debate, invited commentary, or author viewpoint. **Do NOT use** merely because PubMed has no abstract, or for a substantive research-methods framework/consensus framework article. |
...
| **Innovation in Study Design or Analysis** | Novel statistical methods, prediction model development, novel trial designs, estimand frameworks, reporting guidance, or methodological frameworks for designing, analyzing, or interpreting research |
...
Note: The examples below include reasoning for illustration only. Your output must be JSON only.
...
### Example 13: Research Methods Framework, Not Clinical Practice Guideline

**Title:** CRT-Estimands Framework: consensus based extension of the ICH E9(R1) addendum for cluster randomised trials.

**Abstract:** Interest holders need a clear description of a trial's research question. The ICH E9(R1) addendum set out a framework for defining estimands: a precise description of the treatment effect to be estimated. The addendum primarily focused on individually randomised trials, whereas cluster randomised trials present additional challenges. Therefore, the CRT-Estimands Framework was developed as a consensus based extension for cluster randomised trials. This article presents the framework with explanations and examples of how it can be implemented.

**Reasoning:**

  - **Clinical domain**: No specific disease area or healthcare delivery system is studied, so no Topic Category is needed. Do not use Health Systems simply because interest holders or policy makers may use the results.
  - **What investigators did**: Proposed a substantive research-methods framework, not patient-care recommendations and not an opinion/editorial.
  - **Methodological features**: The paper concerns estimands, cluster randomised trial methodology, and consensus-based development.
...
2.  **No Abstract**: If the abstract is missing or empty:
      * Use the lay summary if it is supplied. If neither the abstract nor the lay summary contains enough detail, classify conservatively from explicit title signals only.
      * Assign appropriate **Topic Categories** based only on explicit disease, population, or care-delivery signals in the title or lay summary.
      * Use "Commentary / Editorial" for **Study Design** only if the title explicitly indicates an editorial, commentary, perspective, opinion, or debate; otherwise return an empty list.
      * If the title or lay summary explicitly describes a framework, reporting standard, estimand, statistical method, trial-design method, or consensus methods paper, assign the relevant **Methodological Focus** tags. Otherwise return an empty list \`[]\` for **Methodological Focus**.
      * Set \`"exclude": false\`.
...
6.  **Research Methods Frameworks**: If a paper develops or explains a framework for trial design, estimands, reporting, statistical analysis, or interpretation of research, do **not** classify it as "Clinical Practice Guideline" unless it gives recommendations for patient care. Do **not** classify it as "Health Systems" merely because the results help clinicians, patients, policy makers, or decision makers interpret research. Do **not** classify it as "Commentary / Editorial" unless it is explicitly an opinion/editorial/perspective piece. Use an empty \`study_design\` array when no study-design tag fits; use "Innovation in Study Design or Analysis"; add "Consensus Methods" only when a consensus process is central.
7.  **Multi-topic**: If a study examines dialysis patients undergoing surgery, assign both "Hemodialysis" and "Perioperative and Surgery".
8.  **Correction**: If the title or abstract indicates "Correction", "Erratum", or "Author Correction", set \`"exclude": true\`.
