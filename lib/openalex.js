...
/**
 * Fetch an abstract for a work by DOI.
 * OpenAlex stores abstracts as an inverted index; this reconstructs the text.
 */
export async function getAbstractByDoi(doi, { timeoutMs = 8000 } = {}) {
  if (!doi) return null
  if (!API_KEY) return null
  const cleanDoi = doi.replace(/^https?:\/\/(dx\.)?doi\.org\//i, '').replace(/^doi:\s*/i, '')
  ...
  try {
    const response = await fetch(
      `${BASE_URL}/works/doi:${encodeURIComponent(cleanDoi)}?api_key=${encodeURIComponent(API_KEY)}`,
      { headers: HEADERS, signal: controller.signal }
    )
    if (!response.ok) return null
    const data = await response.json()
    const idx = data?.abstract_inverted_index
    ...
    return keys.map((k) => positions[k]).join(' ')
  } catch {
    return null
  } finally {
    clearTimeout(timer)
