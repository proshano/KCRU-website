...
import { isPublicationExcludedForResearcher, normalizeResearcherPublicationExclusion } from './researcherPublicationIdentity.js'
...
function normalizeEvidence(evidence = {}) {
  return {
    ...
    hasNameConflict: Boolean(evidence.hasNameConflict),
...
export async function upsertPublicationAttributionCandidates({
  ...
  for (const candidate of candidates) {
    ...
    // Correcting a legacy DOI must update the existing review, preserving its
    // manual decision and notification history instead of creating a new pending row.
    if (candidate.review?._id) document._id = candidate.review._id
...
export function filterRejectedProvenance({ provenance = {}, researchers = [], reviews = [], rejectedAttributions = [], excludePending = false } = {}) {
  ...
  const rejectedPairs = new Set(rejectedAttributions.map(({ researcherId, publicationKey }) => getAttributionPairKey(researcherId, publicationKey)))
  const filtered = {}
  ...
  for (const [publicationKey, researcherIds] of Object.entries(provenance || {})) {
    const retainedIds = Array.from(new Set(Array.from(researcherIds || []))).filter((researcherId) => {
      ...
      return researcher && !(excludePending && review?.status === 'pending') &&
        !rejectedPairs.has(getAttributionPairKey(researcherId, publicationKey)) &&
        !isAttributionRejected({ researcher, publicationKey, review })
    })
...
// A search hit is candidate discovery, not proof of identity. Check the indexed
// byline/collaborators before letting it publish or train the coauthor fingerprint.
export function vetPubmedAttributions({ publications = [], provenance = {}, researchers = [], reviews = [] } = {}) {
  const researcherById = new Map(researchers.map((researcher) => [researcher._id, researcher]))
  const reviewByPair = indexPublicationAttributionReviews(reviews)
  ...
  for (const publication of publications) {
    ...
    for (const researcherId of Array.from(provenance[publicationKey] || [])) {
      const researcher = researcherById.get(researcherId)
      ...
      if (evaluation.decision === 'rejected') {
        rejectedAttributions.push({ researcherId, publicationKey })
        continue
      }
      if (review || evaluation.decision === 'hold') candidates.push({ researcher, publication, evaluation, review })
      if (evaluation.decision !== 'confirmed') continue
      if (!confirmedProvenance[publicationKey]) confirmedProvenance[publicationKey] = []
      confirmedProvenance[publicationKey].push(researcherId)
      if (review?.status === 'pending') resolutions.push({ review, reason: evaluation.reason })
    }
  }

  return {
    publications: publications.filter((publication) => confirmedProvenance[getPublicationKey(publication)]?.length),
...
export function mergeApprovedReviewSnapshots({ publications = [], provenance = {}, reviews = [], researchers = [] } = {}) {
  ...
  for (const review of reviews) {
    ...
    const publicationKey = getPublicationKey(review.snapshot)
    const researcherId = review?.researcher?._ref || review.researcherId
    const researcher = researcherById.get(researcherId)
    if (!publicationKey || !researcher || isAttributionRejected({ researcher, publicationKey, review })) continue
    if (!byKey.has(publicationKey)) byKey.set(publicationKey, withPublicationKey(review.snapshot))
    nextProvenance[publicationKey] ||= []
    if (!nextProvenance[publicationKey].includes(researcherId)) {
      nextProvenance[publicationKey].push(researcherId)
    }
  }
