...
export async function generateMissingSummaries({
  ...
  let publications = cachedPublications
  let classificationUpdates = []

  try {
    const merged = await mergeWithClassifications([...cachedPublications])
    const cachedByPmid = new Map(cachedPublications.map(p => [p.pmid, p]))
...
function buildDisplay(publications) {
  ...
  publications.sort(comparePublicationsByDisplayDate)
