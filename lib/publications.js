...
export async function refreshPubmedCache({
  ...
  return withCacheLock(async () => {
    ...
    // Track if we need to write cache (only if something changed)
    let newSummariesGenerated = 0
    const newPublicationsCount = (latest.publications || []).length - existingSummaries.size
    const hasNewPublications = newPublicationsCount > 0
    ...
    if (summariesPerRun > 0 && hasLlmKey && newPapersWithoutSummary > 0) {
      ...
      console.info('[pubmed] Starting summary generation...')
      ...
      newSummariesGenerated = Math.max(0, afterCount - beforeCount)
      summariesMeta = {
        ...summariesMeta,
    ...
    if (!hasNewPublications && newSummariesGenerated === 0) {
    ...
    console.info('[pubmed] Writing cache...', {
      publications: publicationsWithSummaries.length,
      hasNewPublications,
      newSummariesGenerated
    })
