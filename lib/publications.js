...
import { readCache, writeCache, withCacheLock, isCacheStale, CACHE_PATH, isCancelRequested } from './pubmedCache.js'
...
export async function refreshPubmedCache({
  ...
  return withCacheLock(async () => {
    ...
    let publicationsWithSummaries = latest.publications || []
    let summariesMeta = {
      attempted: 0,
      generated: publicationsWithSummaries.filter(p => p.laySummary).length,
      totalWithSummary: publicationsWithSummaries.filter(p => p.laySummary).length,
    ...
    if (summariesPerRun > 0 && hasLlmKey) {
      ...
      summariesMeta = {
        ...
        attempted: Math.min(
          summariesPerRun,
          publicationsWithSummaries.filter(p => p.abstract && p.abstract.length >= 50).length
        ),
...
export async function getCachedPublicationsDisplay({
  ...
  const refreshed = await refreshPubmedCache({
    researchers,
    affiliation,
    maxPerResearcher,
    maxAffiliation,
