...
import { fetchAbstractFromDoi, closeBrowserAfterBatch } from './doiAbstract.js'
...
function pickBestAbstract(primary, fallback) {
...
function hasUsableAbstract(pub) {
...
async function backfillMissingAbstracts(publications = [], { maxItems = Infinity, enabled } = {}) {
  ...
  const candidates = publications
    .filter((pub) => pub?.pmid && pub?.doi && !hasUsableAbstract(pub))
  ...
  try {
    for (let i = 0; i < toFetch.length; i += 1) {
      ...
      if (abstract && abstract.length >= ABSTRACT_MIN_LENGTH) {
        updates.set(pub.pmid, { ...pub, abstract })
      }
  ...
  const updatedPublications = publications.map((pub) => updates.get(pub.pmid) || pub)
...
export async function refreshPubmedCache({
  ...
  return withCacheLock(async () => {
    ...
    const existingPublicationsByPmid = buildPublicationIndex(existingPublications)
    ...
    let publicationsWithSummaries = (latest.publications || []).map(pub => {
      ...
      const merged = {
        ...pub,
        abstract: pickBestAbstract(pub.abstract, existing.abstract),
    ...
    const updatedPublicationsByPmid = new Map()
      ...
      if (abstractUpdates > 0) {
        ...
        for (const pub of backfill.updatedPublications) {
          if (existingPmids.has(pub.pmid)) {
            updatedPublicationsByPmid.set(pub.pmid, pub)
          }
        }
      }
    }

    const newPublicationCount = publicationsWithSummaries.filter((pub) => !existingPmids.has(pub.pmid)).length
    ...
    if (existingPublicationsByPmid.size > 0) {
      for (const pub of publicationsWithSummaries) {
        if (!existingPmids.has(pub.pmid)) continue
        const existing = existingPublicationsByPmid.get(pub.pmid)
        if (existing && hasPublicationDateChanged(existing, pub)) {
          metadataUpdates += 1
          updatedPublicationsByPmid.set(pub.pmid, pub)
        }
      }
    ...
    const newPublications = publicationsWithSummaries.filter((pub) => !existingPmids.has(pub.pmid))
    const updatedPublications = Array.from(updatedPublicationsByPmid.values())
    const hasMetadataUpdates = metadataUpdates > 0 || abstractUpdates > 0
...
export async function addLaySummaries(publications = [], options = {}) {
  ...
  try {
    ...
    return publications.map(pub => ({
      ...
      exclude: isPublicationExcluded({
        ...pub,
        exclude: pub.exclude === true || summaries.get(pub.pmid)?.exclude === true
      })
    }))
  } catch (err) {
...
export async function generateMissingSummaries({
  ...
  try {
    ...
    const cachedByPmid = new Map(publications.map(p => [p.pmid, p]))
    classificationUpdates = merged.filter((pub) => {
      const original = cachedByPmid.get(pub.pmid)
  ...
  for (const pub of allUpdatedPublications) {
    if (pub?.pmid) dedupedUpdates.set(pub.pmid, pub)
  }
