...
export async function generateMissingSummaries({
  ...
  const needsSummary = cached.publications.filter(
    p => !p.laySummary && p.abstract && p.abstract.length >= 50
  )

  if (needsSummary.length === 0) {
    console.info('[pubmed] All publications already have summaries')
    const totalWithSummary = cached.publications.filter(p => p.laySummary).length
    return { attempted: 0, generated: 0, totalWithSummary, alreadyComplete: true }
  }

  console.info('[pubmed] Publications needing summaries', {
    total: cached.publications.length,
    needsSummary: needsSummary.length,
    maxSummaries
  })

  // Sort by most recent first, limit to maxSummaries
  ...
  if (!hasLlmKey) {
    ...
    return { attempted: 0, generated: 0, error: 'No LLM API key' }
  }

  const updatedPublications = await addLaySummaries(toProcess, {
    ...llmOptions,
    maxSummaries,
    summaryOrder: 'recent'
  })

  // Find which ones got summaries
  const newlySummarized = updatedPublications.filter(p => p.laySummary)

  if (newlySummarized.length === 0) {
    console.info('[pubmed] No summaries generated')
    return { attempted: toProcess.length, generated: 0 }
  }

  // Write back incrementally
  console.info('[pubmed] Writing summaries incrementally', { count: newlySummarized.length })
  await writeCacheIncremental({
    newPublications: [],
    updatedPublications: newlySummarized,
    provenance: {},
    meta: { summaries: { model: llmOptions.model } }
  })

  const duration = Date.now() - startTime
  const totalWithSummary = cached.publications.filter(p => p.laySummary).length + newlySummarized.length

  console.info('[pubmed] Summary generation complete', {
    attempted: toProcess.length,
    generated: newlySummarized.length,
    totalWithSummary,
    durationMs: duration
  })
  ...
  return {
    ...
    generated: newlySummarized.length,
    totalWithSummary,
    remaining: needsSummary.length - newlySummarized.length,
    durationMs: duration
  }
}
