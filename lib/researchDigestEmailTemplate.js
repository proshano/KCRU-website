...
export function buildResearchDigestEmail({
  ...
  const subject = applyTemplate(settings.subjectTemplate || 'KCRU kidney research digest - {{date}}', tokens)
  const intro = applyTemplate(
    papers.length || opportunities.length
      ? settings.introText || 'Here is the approved kidney research digest for {{date}}.'
