...
const TRIAL_SUMMARY_SYSTEM_PROMPT = `You write concise clinical trial summaries for physicians.
Be accurate, clinically precise, and non-promotional. Explain the study purpose, intervention, target population,
and key inclusion features without speculation. Keep summaries to 3-5 sentences. Do not make up information.`
...
export async function generateTrialSummary(trialData, options = {}) {
  ...
  const inclusionText = inclusionCriteria?.length
    ? `Key Inclusion Criteria:\n${inclusionCriteria.slice(0, 10).map(c => `- ${c}`).join('\n')}`
    : 'Key Inclusion Criteria: Not specified'
...
Focus on the study purpose, intervention, and who the study includes. Avoid marketing language.`
