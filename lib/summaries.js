...
- Do not ask for or mention UPCR/UACR in this workflow.
...
export async function generateTrialMatchConversation(input = {}, options = {}) {
  ...
  const promptParts = [
    ...
    'Set ready_for_matching to true when major inclusion characteristics are captured (typically within 2–3 exchanges after the first facts): primary diagnosis, age/sex when relevant, populationTags, and eGFR when relevant. Do not require dialysis or transplant status unless the user volunteers it or matching cannot proceed without it.',
  ...
  try {
    ...
    return {
      assistantReply:
        assistantReply || 'Share primary kidney diagnosis and eGFR if known.',
    ...
    return {
      assistantReply:
        'Share non-identifying details only: diagnosis and eGFR.',
