...
export const SOCIAL_POST_SYSTEM_PROMPT = `You write posts for the X (Twitter) account of a kidney clinical research unit. Each post announces one of the team's newly published papers to patients, clinicians and researchers.
Rules:
- State the paper's main finding or aim in plain language, faithful to the lay summary. Do not exaggerate, and do not claim causes, effects or benefits that the lay summary does not state.
- Do not use hype such as "breakthrough", "game-changing" or "groundbreaking", and do not give clinical or medical advice.
- Do not use emojis, hashtags, @handles or links. The link to the paper is added after your text automatically.
- When a journal is given, name it exactly as given.
- You may mention one or two of the listed team members by name if it reads naturally. Never invent names.
- Stay within 240 characters.
Output only the post text, with no quotation marks, labels or commentary.`
 ...
 * `maxLength` (default 240) and `previousText` (a draft that came out too long) let the
 * caller ask once for a shorter version. Returns null on failure or empty output.
 */
export async function generateSocialPostText(input = {}, options = {}) {
  ...
  const journal = normalizePromptValue(input.journal)
  const teamMembers = normalizePromptList(input.teamMembers)
  const siteTitle = normalizePromptValue(input.siteTitle)
  ...
  const promptParts = [
    `Account: ${siteTitle || 'a kidney clinical research unit'}`,
    `Paper title: ${title || 'Not available'}`,
    `Journal: ${journal || 'Not given'}`,
    `Team members among the authors: ${teamMembers.length ? teamMembers.join(', ') : 'Not given'}`,
    '',
  ...
  if (previousText) {
    promptParts.push(
      `Your previous draft was too long for X once the paper link is added:\n${previousText}`,
      '',
      `Write a shorter version of at most ${maxLength} characters.`
    )
  } else {
    promptParts.push(`Write the post in at most ${maxLength} characters.`)
  }
  const userPrompt = promptParts.join('\n')
