...
export async function classifyPublication({ title, abstract, laySummary }, options = {}) {
  ...
  for (let attempt = 0; attempt <= retryAttempts; attempt += 1) {
    try {
      ...
      switch (provider) {
        case 'openrouter':
          responseText = await callOpenRouter(userPrompt, { model, systemPrompt, apiKey, maxTokens, responseFormat })
          break
        case 'openai':
...
async function callOpenRouter(userPrompt, { model, systemPrompt, apiKey, maxTokens, temperature, responseFormat }) {
...
Clinical detail:
- Ask in protocol-style terms (same concepts as inclusion criteria). Use Canadian conventional lab units: eGFR in mL/min/1.73 m² (CKD-EPI); urine protein as UPCR (mg/mmol) or UACR (mg/mmol) when quantifying proteinuria/albuminuria—not vague "high protein."
- Map free-text conditions to patient_profile.populationTags using the allowed enum values (e.g. IgA nephropathy → include iga_nephropathy; also use chronic_kidney_disease or glomerular_disease when appropriate).
...
You receive full public study fields (title, summary, inclusion excerpt, and any structured prescreen metadata). Use clinical judgment: exclude trials that clearly target the wrong population (e.g. dialysis-only trials for a predialysis patient, transplant-only trials for a non-transplant patient, disease-specific trials that do not match the stated diagnosis).
...
/**
 * Compact inclusion/exclusion excerpts for the trial-match LLM (studies from Sanity).
 */
export function buildTrialEligibilityCatalogForPrompt(studies = [], options = {}) {
  const maxPerStudy = options.maxPerStudy ?? 600
  ...
  for (const trial of studies) {
    ...
    const incJoined = Array.isArray(trial?.inclusionCriteria)
      ? trial.inclusionCriteria.map((s) => normalizePromptValue(s)).filter(Boolean).join(' ')
      : ''
    const excJoined = Array.isArray(trial?.exclusionCriteria)
      ? trial.exclusionCriteria.map((s) => normalizePromptValue(s)).filter(Boolean).join(' ')
      : ''
    const inc =
      incJoined.length > maxPerStudy ? `${incJoined.slice(0, maxPerStudy).trim()}...` : incJoined
    const excLimit = Math.max(120, Math.floor(maxPerStudy / 2))
    const exc =
      excJoined.length > excLimit ? `${excJoined.slice(0, excLimit).trim()}...` : excJoined
    if (!inc && !exc) continue
    const lines = [`- ${title}`]
    if (inc) lines.push(`  Inclusion (excerpt): ${inc}`)
    if (exc) lines.push(`  Exclusion (excerpt): ${exc}`)
    chunks.push(lines.join('\n'))
  }

  const joined = chunks.join('\n\n')
  return joined ? truncatePromptText(joined, maxTotal) : ''
}
...
export async function generateTrialMatchConversation(input = {}, options = {}) {
  ...
  const apiKey = options.apiKey || process.env.OPENROUTER_API_KEY
  const maxTokens = resolveMaxTokens(options.maxTokens, 700)
  const temperature = resolveTemperature(options.temperature, 0.2)
  ...
  const promptParts = [
    ...
    truncatePromptText(trialCatalog, 5000),
    '',
    'Protocol eligibility excerpts (truncated; phrase questions using the same concepts and units when applicable):',
    eligibilityCatalog || 'No inclusion/exclusion text on file for these studies.',
    '',
    ...
    'Set exclusionScreeningComplete to true once the user has answered whether major exclusion factors are present, even if exclusionTags is empty.',
    'Set ready_for_matching to true when core characteristics are captured (typically within 2–3 exchanges after the first facts): age, sex, populationTags, eGFR, proteinuria/albuminuria as relevant. Do not require dialysis or transplant status unless the user volunteers it or matching cannot proceed without it.',
  ...
  async function callProvider(userPromptValue) {
    switch (provider) {
      case 'openrouter':
        return callOpenRouter(userPromptValue, { model, systemPrompt, apiKey, maxTokens, temperature, responseFormat })
      case 'openai':
  ...
  try {
    ...
    const responseText = await callProvider(userPrompt)
    const obj = extractFirstJsonObject(responseText)
    const assistantReply = cleanAssistantReply(
      pickFirstString(obj, ['assistant_reply', 'assistantReply', 'reply']) || responseText
    )

    return {
      assistantReply:
        assistantReply ||
        'Share primary kidney diagnosis, eGFR, and UPCR or UACR if known.',
    ...
    return {
      assistantReply:
        'Share non-identifying details only: diagnosis, eGFR, UPCR/UACR.',
...
function buildTrialRankingCatalogForPrompt(studies = []) {
  if (!Array.isArray(studies) || !studies.length) return ''
  const lines = studies.map((t) => {
    const prescreen = t.prescreen && typeof t.prescreen === 'object' ? t.prescreen : {}
    const inc = Array.isArray(t.inclusionCriteria) ? t.inclusionCriteria.slice(0, 12).join(' | ') : ''
    return JSON.stringify({
      _id: t._id,
      title: t.title,
      laySummary: truncatePromptText(t.laySummary || '', 400),
      inclusion_excerpt: truncatePromptText(inc, 900),
      prescreen_populationTags: prescreen.populationTags || [],
      prescreen_dialysis: prescreen.dialysisStatus || null,
      prescreen_transplant: prescreen.transplantStatus || null,
    })
  })
...
function mapLlmRankingRowToMatchResult(trial, relevance, oneLineReason) {
  const prescreen = trial.prescreen && typeof trial.prescreen === 'object' ? trial.prescreen : {}
  ...
  return {
    ...
    decision,
    screeningSummary: prescreen.screeningSummary || trial.laySummary || '',
