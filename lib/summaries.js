...
Avoid marketing language.`
  ...
  try {
    ...
    switch (provider) {
      case 'openrouter':
        result = await callOpenRouter(userPrompt, { model, systemPrompt, apiKey, maxTokens })
        break
      case 'openai':
        result = await callOpenAI(userPrompt, { model, systemPrompt, apiKey, maxTokens })
        break
      default:
        result = await callOpenRouter(userPrompt, { model, systemPrompt, apiKey, maxTokens })
    }
    return sanitizeSummary(nctId, result, { maxSentences: 5 })
  } catch (error) {
