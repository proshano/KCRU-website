...
async function callOpenRouter(userPrompt, { model, systemPrompt, apiKey, maxTokens }) {
...
async function callOpenAI(userPrompt, { model, systemPrompt, apiKey, maxTokens }) {
  const resolvedMaxTokens = resolveMaxTokens(maxTokens, DEFAULT_MAX_TOKENS)
...
async function callTogether(userPrompt, { model, systemPrompt, apiKey, maxTokens }) {
...
const ELIGIBILITY_OVERVIEW_SYSTEM_PROMPT = `You create referral-focused eligibility overviews from detailed clinical trial criteria.
Your goal is to answer: "Who might this study be relevant for, and who is it clearly not for?"

Retain criteria that are stable and fundamental (core diagnosis, disease stage, dialysis modality), immutable or very slow to change,
easy to confirm from a patient encounter, or true absolute contraindications (active malignancy, pregnancy if teratogenic agent, life expectancy < 6 months).

Omit or soften criteria that are labile (labs, eGFR fluctuations), medication-related with feasible washout, time-based (they could become eligible),
require deep chart review, or are assessed at screening (capacity to consent, willingness to comply).

Use clinical reasoning: if an exclusion implies a fixed condition (e.g., anticoagulation for a mechanical valve), mention the condition.
If age is an important criterion, mention it inline; do not create a separate age section.
Avoid emphasizing timelines. If time-based criteria exist, mention that waiting periods may apply rather than listing specific time windows.
Be accurate, clear, and non-promotional.`
...
Write a 2-3 sentence plain-language summary of this study for patients.`
  ...
  try {
    ...
    switch (provider) {
      case 'openrouter':
        result = await callOpenRouter(userPrompt, { model, systemPrompt: TRIAL_SUMMARY_SYSTEM_PROMPT, apiKey, maxTokens })
        break
      case 'openai':
        result = await callOpenAI(userPrompt, { model, systemPrompt: TRIAL_SUMMARY_SYSTEM_PROMPT, apiKey, maxTokens })
        break
      default:
        result = await callOpenRouter(userPrompt, { model, systemPrompt: TRIAL_SUMMARY_SYSTEM_PROMPT, apiKey, maxTokens })
    }
...
export async function generateEligibilityOverview(eligibility, options = {}) {
  ...
  const { inclusionCriteria, exclusionCriteria } = eligibility
...
Write an eligibility overview for referral decisions in 2-4 short sentences.
Use labels "May be relevant for:", "Not for:", and "Note:" in the same paragraph.
Do not use "In brief".
Do not use markdown headings or bullet lists.`
  ...
  try {
    ...
    switch (provider) {
      case 'openrouter':
        result = await callOpenRouter(userPrompt, { model, systemPrompt: ELIGIBILITY_OVERVIEW_SYSTEM_PROMPT, apiKey, maxTokens })
        break
      case 'openai':
        result = await callOpenAI(userPrompt, { model, systemPrompt: ELIGIBILITY_OVERVIEW_SYSTEM_PROMPT, apiKey, maxTokens })
        break
      default:
        result = await callOpenRouter(userPrompt, { model, systemPrompt: ELIGIBILITY_OVERVIEW_SYSTEM_PROMPT, apiKey, maxTokens })
    }
