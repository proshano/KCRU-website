...
export async function generateTrialMatchConversation(input = {}, options = {}) {
  ...
  const promptParts = [
    ...
    'Set ready_for_matching to true when the main inclusion characteristics are captured (typically within 2–3 exchanges after the first facts): primary diagnosis, populationTags when helpful, and eGFR when relevant. Age and sex are optional unless a remaining study clearly depends on them. Do not require dialysis or transplant status unless the user volunteers it or matching cannot proceed without it.',
