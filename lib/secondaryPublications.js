...
function matchesResearcherName(authors = [], researcherName = '', researcherOrcid = '') {
  ...
  return authors.some((author) => {
    ...
    return given.length === 1 ? given === expectedGiven[0] : given.split(' ')[0] === expectedGiven
  })
}
...
export async function fetchCrossrefPublications(researcher, options = {}) {
  ...
  const name = String(researcher?.name || '').trim()
  ...
  for (const params of requestParams) {
    ...
    const isNameQuery = params.has('query.author')
    const items = data?.message?.items || []
    publications.push(...items
      .filter((item) => !isNameQuery || matchesResearcherName(item?.author || [], name, orcid))
