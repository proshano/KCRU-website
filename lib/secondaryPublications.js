...
export async function fetchCrossrefPublications(researcher, options = {}) {
  ...
  for (const { params, queryPath } of requests) {
    ...
    publications.push(...items
      ...
      .filter((item) => item?.title && item?.year >= sinceYear && !isPublicationSupplementaryMaterial(item)))
  }
...
export async function fetchOpenAlexPublications(researcher, options = {}) {
  ...
  return (data?.results || [])
    ...
    .filter((item) => item?.title && item?.year >= sinceYear && !isPublicationSupplementaryMaterial(item) && matchesResearcherAuthorList(item.authors, name))
}
