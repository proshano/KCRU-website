...
function resolveIntro({ settings, hasPublications, monthLabel, rangeLabel, count, publicationStats }) {
  ...
  if (hasPublications) {
    const templated = applyTemplate(settings?.introText, tokens)
    if (templated) return templated
    return rangeLabel
  ...
  const emptyTemplated = applyTemplate(settings?.emptyIntroText, tokens)
...
export function buildPublicationNewsletterEmail({
  ...
  const html = `
    <div style="font-family: system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif; font-size: 14px; color: #111; line-height: 1.5; background: #ffffff;">
      ...
      <p style="margin: 0 0 18px; color: ${MUTED_TEXT}; font-size: 15px; line-height: 1.6;">
        ${escapeHtml(intro)}
      </p>

      ${htmlBlocks}

      ${outro ? `<p style="margin: 16px 0 6px; color: #111; font-size: 15px; line-height: 1.6;">${escapeHtml(outro)}</p>` : ''}
