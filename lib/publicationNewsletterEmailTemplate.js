...
function buildInvestigatorLinksHtml(pub, researchers, provenance, siteBaseUrl) {
  ...
  const links = matches.map((researcher) => {
    ...
    if (href) {
      return `<a href="${escapeHtml(href)}" style="color: ${BRAND_COLOR} !important; text-decoration: underline !important; font-weight: 600;">${escapeHtml(name)}</a>`
    }
...
function buildShareLinksHtml(pub) {
  ...
  const textLinks = buttons.map((btn) => `
    <a href="${escapeHtml(btn.url)}" target="_blank" rel="noopener noreferrer"
      style="color: ${BRAND_COLOR}; text-decoration: underline; font-weight: 600; font-size: 15px; margin-right: 16px;">
      ${escapeHtml(btn.label || 'Share')}
    </a>
  `)
  return `<div style="margin: 8px 0 10px;">${textLinks.join('')}</div>`
}
...
function buildPublicationHtmlBlock({ pub, researchers, provenance, siteBaseUrl }) {
  ...
  return `
    <div style="background: ${CARD_BACKGROUND}; border: 1px solid ${CARD_BORDER_COLOR}; border-radius: 12px; padding: 16px; margin: 0 0 18px; box-shadow: 0 1px 2px rgba(15, 23, 42, 0.08);">
      <h2 style="margin: 0 0 8px; font-size: 18px; line-height: 1.45; color: #111; font-weight: 700;">
        ${link ? `<a href="${escapeHtml(link)}" style="color: #111; text-decoration: underline; font-weight: 700;">${escapeHtml(title)}</a>` : escapeHtml(title)}
      </h2>
