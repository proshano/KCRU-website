...
import { getShareButtons } from '@/lib/sharing'
...
function buildPublicationLink(pub, baseUrl) {
  const rawLink = pub?.pubmedUrl
    || (pub?.pmid ? `https://pubmed.ncbi.nlm.nih.gov/${pub.pmid}/` : '')
    || (pub?.doi ? `https://doi.org/${pub.doi}` : '')
    || pub?.url
    || ''
  if (!rawLink) return ''
  return buildOutboundRedirectUrl(rawLink, { baseUrl })
}
...
function buildShareLinksHtml(pub, baseUrl) {
  const buttons = getShareButtons(pub).map((btn) => ({
    ...btn,
    url: buildOutboundRedirectUrl(btn.url, { baseUrl }),
  }))
...
function buildPublicationHtmlBlock({ pub, researchers, provenance, siteBaseUrl }) {
  ...
  const link = buildPublicationLink(pub, baseUrl)
  ...
  const shareLinksHtml = buildShareLinksHtml(pub, baseUrl)
...
function buildPublicationTextBlock({ pub, researchers, provenance, siteBaseUrl }) {
  ...
  const shareLines = shareButtons.map((btn) => `${btn.label}: ${buildOutboundRedirectUrl(btn.url, { baseUrl })}`)
