...
function buildPublicationHtmlBlock({ pub, researchers, provenance, siteBaseUrl }) {
  ...
  const { authors, meta } = buildCitation(pub)
  const summary = sanitizeText(pub?.laySummary)
  const summaryHtml = summary
    ? `<p style="margin: 0; font-size: 13px; color: #374151; background: #f5f3f0; border: 1px solid #eee; border-radius: 8px; padding: 12px;">${escapeHtml(summary)}</p>`
    : `<p style="margin: 0; font-size: 13px; color: ${MUTED_TEXT}; background: #f9fafb; border: 1px solid ${BORDER_COLOR}; border-radius: 8px; padding: 12px;">Summary pending.</p>`

  return `
    <div style="border: 1px solid ${BORDER_COLOR}; border-radius: 12px; padding: 16px; margin-bottom: 16px;">
      ...
      ${authors ? `<p style="margin: 0 0 4px; font-size: 12px; color: #4b5563;">${escapeHtml(authors)}</p>` : ''}
...
function buildPublicationTextBlock({ pub, researchers, provenance, siteBaseUrl }) {
  ...
  const lines = [
    ...
    meta,
    investigatorLine,
    summary ? `Summary: ${summary}` : 'Summary: pending.',
