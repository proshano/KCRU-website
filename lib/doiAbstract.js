...
import { browserFetchBytes, browserFetchHtml, closeBrowser } from './browserFetch.js'
...
/**
 * Last resort for records the publisher shows only as a picture of the opening page:
 * editorials, commentaries and letters behind a paywall, whose page carries no abstract
 * and no article text. Finds the preview image, downloads it and has the configured
 * vision model transcribe it. The text is stored as article body text (it is not an
 * abstract) with a source marking it as the first page only.
 *
 * `fetchImage` is injectable for tests; the browser fetcher is offered as the fallback
 * for an image request that publisher bot protection refuses.
 */
export async function transcribePublisherFirstPage({
  renderedHtml,
  plainHtml,
  pageUrl,
  title,
  transcribeImage,
  fetchImage = fetchPublisherFirstPageImage,
  browserFetch = BROWSER_FETCH_ENABLED ? browserFetchBytes : undefined,
} = {}) {
  if (typeof transcribeImage !== 'function') return null

  const imageUrl = extractFirstPageImageUrl(renderedHtml, { baseUrl: pageUrl })
    || extractFirstPageImageUrl(plainHtml, { baseUrl: pageUrl })
  if (!imageUrl) return null

  console.info(`[doi-abstract] Found first-page image for ${pageUrl}`, { imageUrl })
  const image = await fetchImage(imageUrl, { browserFetch })
  if (!image?.bytes?.byteLength) return null

  const transcript = await transcribeImage({ image: image.bytes, mimeType: image.mimeType, title, url: imageUrl })
  const text = normalizePublicationText(transcript)
  if (!text || text.length < MIN_ARTICLE_BODY_LENGTH) {
    console.warn(`[doi-abstract] First-page transcription too short for ${pageUrl}`, {
      chars: text?.length || 0,
    })
    return null
  }

  return { text: truncateArticleBody(text), contentType: 'article_body', source: PUBLISHER_FIRST_PAGE_SOURCE }
}
 ...
 */
async function fetchPublisherPageContent(url, plainHtml, { pageUrl, title, transcribeImage } = {}) {
  ...
  return transcribePublisherFirstPage({
    renderedHtml,
    plainHtml,
    pageUrl: pageUrl || url,
    title,
 ...
 * in order before rendered article body text is accepted, and a transcription of the
 * publisher's first-page preview image comes last, only when `transcribeImage` is given.
 *
  ...
  const sources = sourceFetchers || [
    {
      ...
      fetch: async () => {
        ...
        publisherHtml = fetched?.text || null
        publisherUrl = fetched?.finalUrl || null
    ...
    {
      name: 'publisher page',
      fetch: () => fetchPublisherPageContent(url, publisherHtml, {
