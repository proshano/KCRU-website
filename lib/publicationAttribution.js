...
export function classifyResearcherAuthor(author = {}, researcher = {}) {
  ...
  for (const name of names) {
    const match = classifyAuthorName(author, getResearcherPublicationNameParts({ name }))
...
function classifyAuthorName(author, expected) {
  ...
  if (!(family === expected.family || family.endsWith(` ${expected.family}`)) || !givenParts.length) return null
