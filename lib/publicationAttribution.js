...
export function normalizeAttributionAuthor(author = {}, source = '') {
  ...
  return {
    ...
    ...(author.role === 'investigator' ? { role: 'investigator' } : {}),
  }
}

export function getPublicationAttributionAuthors(publication = {}, { includeIncomplete = false } = {}) {
  ...
  if (structured.length) {
    return structured
      ...
      .filter((author) => author.family && (author.given || includeIncomplete))
  }
...
export function preferPubmedAttributionMetadata(publication, pubmedPublication) {
  if (!pubmedPublication?.attributionAuthors?.length) return publication
  // Secondary author clusters can substitute a namesake or omit consortium members.
  // Retain the actual indexed byline and collaborators when PubMed supplies them.
  return {
    ...publication,
    ...pubmedPublication,
    sources: Array.from(new Set([...(publication.sources || []), ...(pubmedPublication.sources || []), 'pubmed'])),
    attributionQueryPaths: Array.from(new Set([
      ...(publication.attributionQueryPaths || []),
      ...(pubmedPublication.attributionQueryPaths || []),
    ])),
  }
}

function getResearcherPublicationNames(researcher) {
  const names = [researcher.publicationAuthorName || researcher.name, ...(researcher.publicationAuthorAliases || [])]
  return names.map((name) => getResearcherPublicationNameParts({ name }))
}

function hasMatchingFamilyName(author, expected) {
  const family = normalizeAttributionText(author.family)
  return Boolean(expected.family && (family === expected.family || family.endsWith(` ${expected.family}`)))
}

export function classifyResearcherAuthor(author = {}, researcher = {}) {
  for (const expected of getResearcherPublicationNames(researcher)) {
    const match = classifyAuthorName(author, expected)
    if (match) return match
  }
  return null
}

function hasConflictingAuthorName(authors, researcher) {
  const expectedNames = getResearcherPublicationNames(researcher)
  const sameFamilyAuthors = authors.filter((author) => expectedNames.some((expected) => hasMatchingFamilyName(author, expected)))
  // An absent contributor or a surname without a given name is incomplete evidence.
  // Explicitly different names are enough to discard a hit only when no compatible
  // author or investigator remains, including any verified publication aliases.
  return sameFamilyAuthors.length > 0 && sameFamilyAuthors.every((author) => {
    return normalizeAttributionText(author.given) && !classifyResearcherAuthor(author, researcher)
  })
}

function classifyAuthorName(author, expected) {
  const normalizedAuthor = normalizeAttributionAuthor(author)
  const given = normalizedAuthor.given.trim()
  // PubMed and Europe PMC also return compact initials such as MA, not just M. A.
  const givenParts = /^[A-Z]{1,4}$/.test(given)
    ? given.toLowerCase().split('')
    : normalizeAttributionText(given).split(' ').filter(Boolean)
  if (!hasMatchingFamilyName(normalizedAuthor, expected) || !givenParts.length) return null
...
function addFingerprintPublication(fingerprint, publication, { countCoauthors }) {
  ...
  for (const author of authors) {
    if (author.role === 'investigator') continue
    const signature = getAttributionAuthorSignature(author)
    if (!signature || signature === researcherSignature || counted.has(signature)) continue
...
export function decideAttributionEvidence({
  ...
  if (hasNameConflict && !nameKind) {
    return { decision: 'rejected', reason: 'author name conflicts with the researcher publication names' }
  }
  if (hasExactOrcid && nameKind) {
  ...
  if (isPubmedConfirmed && nameKind) {
    return { decision: 'confirmed', reason: 'compatible indexed PubMed author or collaborator' }
  }
...
export function evaluatePublicationAttribution({
  ...
  const authors = getPublicationAttributionAuthors(publication, { includeIncomplete: true })
  ...
  const nameMatches = authors
    ...
    .filter(Boolean)
  const hasNameConflict = !nameMatches.length && hasConflictingAuthorName(authors, researcher)
  const exactOrcidAuthor = expectedOrcid
    ? nameMatches.find((author) => normalizeAttributionOrcid(author.orcid) === expectedOrcid)
    : null
...
export function getAttributionPairKey(researcherId, publicationKey) {
  const storedKey = String(publicationKey || '').trim()
  const doi = storedKey.startsWith('doi:') ? normalizeDoi(storedKey) : ''
  const key = doi ? `doi:${doi}` : storedKey
  return `${String(researcherId || '').trim()}|${key}`
}
