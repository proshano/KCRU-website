...
const SEX_OPTIONS = new Set(['all', 'female', 'male'])
...
export function normalizeStudyPayload(body) {
  ...
  const sex = normalizeEnum(payload.sex, SEX_OPTIONS)
  ...
  return {
    ...
    whatToExpect: sanitizeString(payload.whatToExpect),
    duration: sanitizeString(payload.duration),
    compensation: sanitizeString(payload.compensation),
    ...
    recruitmentSiteIds: uniqueArray(normalizeList(payload.recruitmentSiteIds)),
