...
import { buildPublicationClassificationPatch, resolvePublicationClassificationSettings } from './classificationSettings.js'
...
import { isPublicationExcludedByRule } from './publicationExclusions.js'
...
function buildPaperRecord({ publication, baseline }, jevResult, error) {
  ...
  const pmid = String(publication.pmid)
  const publicationTypeExcluded = isPublicationExcludedByRule(publication)
  const jev = jevResult
    ? {
...
export async function runClassificationEval(options = {}) {
  ...
  // Runs start from the production thresholds so the page's sliders show what the site
  // would do today; explicit options still win.
  const production = options.settings
    ? resolvePublicationClassificationSettings(options.settings)
    : await fetchPublicationClassificationSettings(client)
  const thresholds = resolveJevThresholds({ ...production.thresholds, ...(options.thresholds || {}) })
...
const SETTINGS_QUERY = `*[_type == "siteSettings"][0]{ _id, publicationClassification }`

/** The staff-facing classifier settings, with the thresholds Jev would actually use. */
export async function fetchPublicationClassificationSettings(client) {
  const doc = await client.fetch(SETTINGS_QUERY)
  const resolved = resolvePublicationClassificationSettings(doc && !Array.isArray(doc) ? doc : {})
  return {
    ...resolved,
    effectiveThresholds: resolveJevThresholds(resolved.thresholds),
  }
}

/** Write thresholds and/or the backend to siteSettings.publicationClassification. */
export async function patchPublicationClassificationSettings(client, input = {}) {
  const current = await client.fetch(SETTINGS_QUERY)
  if (!current?._id) {
    throw new Error('No siteSettings document exists yet. Create one in Sanity Studio first.')
  }
  const next = buildPublicationClassificationPatch(input, current.publicationClassification || {})
  await client.patch(current._id).set({ publicationClassification: next }).commit({ returnDocuments: false })
  const resolved = resolvePublicationClassificationSettings({ publicationClassification: next })
  return { ...resolved, effectiveThresholds: resolveJevThresholds(resolved.thresholds) }
}
