...
export function comparePublicationsByDisplayDate(a, b) {
  ...
  const pmidA = Number.parseInt(a?.pmid, 10)
  const pmidB = Number.parseInt(b?.pmid, 10)
  if (!Number.isNaN(pmidA) && !Number.isNaN(pmidB) && pmidA !== pmidB) {
    return pmidB - pmidA
  }
...
export function findResearchersForPublication(pub, researchers = [], provenance = {}) {
  ...
  const fromProvenance = new Set(getProvenanceIds(pub, provenance))
