...
// Singleton doc (fixed _id, same string as the type) holding the approver's custom
// drafting prompt. Kept separate from siteSettings so a portal save never collides with
// an unrelated Studio draft of siteSettings.
export const SOCIAL_POST_PROMPT_TYPE = 'socialPostingPrompt'
export const SOCIAL_NETWORK_X = 'x'
export const X_MAX_WEIGHTED_LENGTH = 280
export const X_URL_WEIGHT = 23
// How posts introduce the team's investigators when no Site Settings value is given.
export const DEFAULT_TEAM_LABEL = 'London Kidney'
...
// How long an investigator whose profile a post linked to waits before the rotation
// picks them again (unless they are the paper's first author).
export const SOCIAL_POST_SPOTLIGHT_WINDOW_DAYS = 30

const URL_PATTERN = /https?:\/\/\S+/g
const PROFILE_LINK_PATTERN = /^https?:\/\/[^/\s?#]+\/team\/([A-Za-z0-9_-]+)/i
const DAY_MS = 24 * 60 * 60 * 1000
// Posts whose text counts as featuring a profile: written, on the way to X, or on X.
const SPOTLIGHT_STATUSES = new Set(['draft', 'sending', 'queued', 'removing', 'published'])
const SPOTLIGHT_REASONS = {
  'first-author': 'first author',
  rotation: `picked at random from the investigators not featured in the last ${SOCIAL_POST_SPOTLIGHT_WINDOW_DAYS} days`,
  'least-recent': `every investigator on this paper was featured in the last ${SOCIAL_POST_SPOTLIGHT_WINDOW_DAYS} days, so this is the one featured longest ago`,
}
const ELLIPSIS = '…'
...
const SPOTLIGHT_FIELDS = ['spotlightResearcherId', 'spotlightName', 'spotlightSlug', 'spotlightReason', 'postLink']
const DRAFT_FIELDS = ['text', 'proposedText', 'generatedBy', 'draftedBy', 'draftedAt', ...SPOTLIGHT_FIELDS]
...
function dedupeCleanNames(names) {
  return Array.from(new Set((names || []).map(cleanString).filter(Boolean)))
}

// Dedupes and cleans a list of names, returning '', 'A', 'A and B' or 'A, B and C'.
export function formatNameList(names) {
  const cleaned = dedupeCleanNames(names)
...
// Whether the paper has authors beyond the listed team members. The approver only wants a
// yes/no/unknown signal for drafting - never a count - so this returns null (unknown) when
// `authors` is missing or has no non-blank entries, otherwise a plain comparison of lengths.
export function computeHasOtherAuthors(authors, teamMembers) {
  const nonBlankAuthors = (Array.isArray(authors) ? authors : []).map(cleanString).filter(Boolean)
  if (!nonBlankAuthors.length) return null
  return nonBlankAuthors.length > (teamMembers || []).length
}

// The template draft, used when the AI draft is unavailable. Names every listed investigator
// as one of the team's own, since that is the approver's key requirement for every post.
// `hasOtherAuthors: true` adds "and colleagues" after the names (or after "investigators"
// when none are listed), unless the byline-shortening below has already added it.
export function buildXPostText({ title, link, teamMembers, teamLabel, hasOtherAuthors } = {}) {
  const label = cleanString(teamLabel) || DEFAULT_TEAM_LABEL
  const paperTitle = cleanString(title)
  const paperLink = cleanString(link)
  const uniqueNames = dedupeCleanNames(teamMembers)
  const addColleagues = hasOtherAuthors === true

  const opening = (names) => {
    const list = formatNameList(names)
    const alreadyNamesColleagues = names[names.length - 1] === 'colleagues'
    const suffix = addColleagues && !alreadyNamesColleagues ? ' and colleagues' : ''
    return list ? `New from ${label} investigators ${list}${suffix}:` : `New from ${label} investigators${suffix}:`
  }

  let names = uniqueNames
  let intro = opening(names)
  let full = [intro, paperTitle, paperLink].filter(Boolean).join(' ')
  if (!paperTitle || xWeightedLength(full) <= X_MAX_WEIGHTED_LENGTH) return full

  let withoutTitle = [intro, paperLink].filter(Boolean).join(' ')
  let titleBudget = X_MAX_WEIGHTED_LENGTH - xWeightedLength(withoutTitle) - 1

  // Not enough room left for a meaningful title: shorten the byline instead of the title.
  if (titleBudget < 40 && uniqueNames.length > 3) {
    names = [...uniqueNames.slice(0, 3), 'colleagues']
    intro = opening(names)
    full = [intro, paperTitle, paperLink].filter(Boolean).join(' ')
    if (xWeightedLength(full) <= X_MAX_WEIGHTED_LENGTH) return full
    withoutTitle = [intro, paperLink].filter(Boolean).join(' ')
    titleBudget = X_MAX_WEIGHTED_LENGTH - xWeightedLength(withoutTitle) - 1
  }

  if (titleBudget < 2) return withoutTitle
  return [intro, truncateToWeight(paperTitle, titleBudget), paperLink].filter(Boolean).join(' ')
}
...
// The approver's custom drafting prompt at /admin/social, stored as plain text.
export const SOCIAL_POST_PROMPT_MAX_LENGTH = 6000

export function validateSocialPostPrompt(text) {
  const value = String(text ?? '').trim()
  if (!value) return { ok: false, error: 'Drafting instructions cannot be empty.' }
  if (value.length > SOCIAL_POST_PROMPT_MAX_LENGTH) {
    return {
      ok: false,
      error: `Drafting instructions are ${value.length} characters; the limit is ${SOCIAL_POST_PROMPT_MAX_LENGTH}.`,
    }
  }
  return { ok: true, error: null }
}

// The prompt actually sent to the model: the approver's custom text when there is one,
// otherwise the built-in default (SOCIAL_POST_SYSTEM_PROMPT in lib/summaries.js).
export function resolveSocialPostSystemPrompt(custom, defaultPrompt) {
  const trimmed = String(custom ?? '').trim()
  return trimmed || String(defaultPrompt ?? '')
}

// --- Profile link ("spotlight") ---------------------------------------------
//
// X cannot hyperlink names, so a post ends with a link to one investigator's profile page,
// anchored to the paper, and X shows that profile's preview card. Who was featured is read
// from each post's text rather than a stored id, so a link edited by hand still counts.

// Lowercased profile slugs of every /team/<slug> link in the text.
export function findProfileSlugs(text) {
  const slugs = new Set()
  for (const url of String(text ?? '').match(URL_PATTERN) || []) {
    const slug = url.match(PROFILE_LINK_PATTERN)?.[1]
    if (slug) slugs.add(slug.toLowerCase())
  }
  return slugs
}

export function postLinksToProfile(text, slug) {
  const wanted = cleanString(slug).toLowerCase()
  return Boolean(wanted) && findProfileSlugs(text).has(wanted)
}

function spotlightTime(record) {
  // An upcoming dueAt counts too: the post is on its way.
  for (const field of ['sentAt', 'dueAt', 'queuedAt', 'draftedAt']) {
    const time = timeOf(record?.[field])
    if (time !== null) return time
  }
  return null
}

// researcherId -> time (ms) of the latest post in the window that links to their profile.
// `researchers` are { _id, slug }. The post being drafted is left out with `excludeId`.
export function collectRecentSpotlights({
  records = [],
  researchers = [],
  now = new Date(),
  windowDays = SOCIAL_POST_SPOTLIGHT_WINDOW_DAYS,
  excludeId,
} = {}) {
  const idsBySlug = new Map()
  for (const researcher of researchers || []) {
    const slug = cleanString(researcher?.slug).toLowerCase()
    if (slug && researcher?._id) idsBySlug.set(slug, researcher._id)
  }
  const cutoff = new Date(now).getTime() - windowDays * DAY_MS
  const latest = new Map()
  for (const record of records || []) {
    if (!record || (excludeId && record._id === excludeId)) continue
    if (!SPOTLIGHT_STATUSES.has(normalizeSocialPostStatus(record.status))) continue
    const time = spotlightTime(record)
    if (time === null || time < cutoff) continue
    for (const slug of findProfileSlugs(record.text)) {
      const id = idsBySlug.get(slug)
      if (id && !(latest.get(id) >= time)) latest.set(id, time)
    }
  }
  return latest
}

function pickAtRandom(items, random) {
  const index = Math.floor(Number(random()) * items.length)
  return items[Math.min(Math.max(index, 0), items.length - 1)] || items[0]
}

// `candidates` are the paper's investigators with a profile: { _id, name, slug }. The first
// author always gets the link; otherwise a random investigator not featured in the window,
// or, when all were, the one featured longest ago (ties at random). Null when none has a profile.
export function chooseSpotlightResearcher({
  candidates = [],
  firstAuthorId,
  recentSpotlights = new Map(),
  random = Math.random,
} = {}) {
  const seen = new Set()
  const eligible = (candidates || []).filter((candidate) => {
    if (!candidate?._id || !cleanString(candidate.slug) || seen.has(candidate._id)) return false
    seen.add(candidate._id)
    return true
  })
  if (!eligible.length) return null

  const firstAuthor = firstAuthorId ? eligible.find((candidate) => candidate._id === firstAuthorId) : null
  if (firstAuthor) return { researcher: firstAuthor, reason: 'first-author' }

  const lastFeatured = (candidate) => recentSpotlights?.get?.(candidate._id)
  const notRecent = eligible.filter((candidate) => lastFeatured(candidate) === undefined)
  if (notRecent.length) return { researcher: pickAtRandom(notRecent, random), reason: 'rotation' }

  const oldest = Math.min(...eligible.map(lastFeatured))
  const longestAgo = eligible.filter((candidate) => lastFeatured(candidate) === oldest)
  return { researcher: pickAtRandom(longestAgo, random), reason: 'least-recent' }
}

export function buildProfilePaperLink({ baseUrl, slug, anchor } = {}) {
  const profileSlug = cleanString(slug)
  if (!profileSlug) return ''
  const base = cleanString(baseUrl).replace(/\/+$/, '')
  const hash = cleanString(anchor)
  return `${base}/team/${encodeURIComponent(profileSlug)}${hash ? `#${hash}` : ''}`
}

export function describeSpotlightReason(reason) {
  return SPOTLIGHT_REASONS[reason] || ''
}
...
// The record fields for the profile a draft links to; all unset when it links to the paper.
function spotlightFields(spotlight) {
  if (!spotlight) return unsetAll(SPOTLIGHT_FIELDS)
  return {
    spotlightResearcherId: cleanString(spotlight.researcherId) || null,
    spotlightName: cleanString(spotlight.name) || null,
    spotlightSlug: cleanString(spotlight.slug) || null,
    spotlightReason: cleanString(spotlight.reason) || null,
    postLink: cleanString(spotlight.link) || null,
  }
}

// available -> draft ("Create post") and draft -> draft ("Regenerate").
// `compose(post)` returns { text, generatedBy, spotlight }, where spotlight is
// { researcherId, name, slug, reason, link } or null; see socialPostDrafting.js and
// resolveSocialPostSpotlight in socialPostingStore.js.
export async function draftSocialPost({
  ...
  const spotlight = spotlightFields(draft.spotlight)
  const fields = {
    status: 'draft',
    text: draft.text,
  ...
  return success(
    ...
    { _id: id, status: 'draft', text: draft.text, generatedBy: draft.generatedBy, ...spotlight }
  )
}
