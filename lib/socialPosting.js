...
export function planSocialPostSync({
  ...
  // Nothing reaches Buffer without an approver's review, so the first sync
  // (no records yet) brings the whole current feed to the approver as
  // pending posts, and the maxNew anomaly guard does not apply there - the
  // feed itself is capped at 50 items. `seed` remains only as an explicit
  // way to mark items handled without review, for recovering from an
  // unexpectedly large sync.
  if (seed) return { mode: 'seed', toSeed: unrecorded }
  if (!records?.length) return { mode: 'create', toCreate: unrecorded }
