...
/**
 * Follow the request through its redirects, validating every hop, and return the final
 * response once its status and content type have been checked. The body is left unread
 * so callers can decode it as text or keep it as bytes.
 */
async function safeFetchResponse(value, {
  controller,
  maxRedirects,
  headers,
  allowedProtocols,
...
export async function safeFetchText(value, {
  ...
  try {
    const { response, finalUrl } = await safeFetchResponse(value, {
      controller, maxRedirects, headers, allowedProtocols, allowedContentTypes, fetchImpl, lookup,
    })
    const text = await readTextWithLimit(response, maxBytes)
    return { response, text, finalUrl }
  } finally {
    clearTimeout(timer)
  }
}
...
/**
 * Binary counterpart of safeFetchText for publisher assets such as page images. The
 * same host, redirect, content-type and size checks apply; the body comes back as a
 * Buffer instead of decoded text.
 */
export async function safeFetchBytes(value, {
  timeoutMs = 8000,
  maxBytes = DEFAULT_MAX_BYTES,
  maxRedirects = DEFAULT_MAX_REDIRECTS,
  headers,
