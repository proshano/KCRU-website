import { client, writeClient } from './sanity.js'
...
async function acquireLock() {
  try {
    ...
    try {
      ...
      console.error('Failed to acquire lock', createErr)
      return false
    }
  }
}
...
export async function getExistingSummaries() {
  try {
    const doc = await client.fetch(
      `*[_type == $type && _id == $id][0].publications[]{ pmid, laySummary }`,
      { type: CACHE_DOC_TYPE, id: CACHE_DOC_ID }
    )
    ...
    for (const pub of doc) {
      if (pub.pmid && pub.laySummary) {
        map.set(pub.pmid, pub.laySummary)
      }
    }
    return map
  } catch (err) {
...
export const CACHE_PATH = 'sanity:pubmedCache'
