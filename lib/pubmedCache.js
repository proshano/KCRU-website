...
import { getPublicationKey, mergePublications, normalizeDoi, normalizePublicationProvenance, toSanityPublicationKey, withPublicationKey } from './publicationIdentity.js'
import { toSanityBackfillFailures } from './doiBackfillHistory.js'
...
function resolveLocalCacheDir() {
  if (process.env.PUBMED_CACHE_DIR) return process.env.PUBMED_CACHE_DIR
  if (process.env.VERCEL || process.env.AWS_LAMBDA_FUNCTION_NAME) {
    return path.join('/tmp', 'runtime')
  }
  return path.join(process.cwd(), 'runtime')
}

// Local file path (for backup and upload workflow)
const LOCAL_CACHE_PATH = path.join(resolveLocalCacheDir(), 'pubmed-cache.json')
...
/**
 * Persist the DOI backfill negative cache on its own.
 *
 * The refresh can finish on any of three write paths (full replace, incremental patch,
 * or the no-changes path that only touches metadata), and a run whose only outcome is
 * "these DOIs still have no abstract" takes the last one. Patching the field directly
 * keeps the history durable without forcing a cache rewrite.
 */
export async function persistDoiBackfillFailures(entries = []) {
  try {
    await writeClient
      .patch(CACHE_DOC_ID)
      .set({ doiBackfillFailures: toSanityBackfillFailures(entries) })
      .commit()
    return true
  } catch (err) {
    console.warn('[pubmed] Failed to persist DOI backfill history', err?.message || err)
    return false
  }
}

export async function touchCacheMetadata({
  key,
  generatedAt,
  totalPublications,
  totalWithSummary,
  ...
  const lastRefreshedAt = generatedAt || new Date().toISOString()
  const patchData = {
    lastRefreshedAt,
  ...
  if (typeof key !== 'undefined') patchData.cacheKey = key
  if (Number.isFinite(totalPublications)) patchData['stats.totalPublications'] = totalPublications
  if (Number.isFinite(totalWithSummary)) patchData['stats.totalWithSummary'] = totalWithSummary
  if (summaryModel) patchData['stats.lastSummaryModel'] = summaryModel
  ...
  try {
    await writeClient
      ...
      .set(patchData)
      .commit()
    console.info('[pubmed] Cache metadata updated', { lastRefreshedAt })
    return true
  } catch (err) {
    console.warn('[pubmed] Failed to update cache metadata', err)
    return false
  }
}
  ...
  try {
    ...
    if (doc.provenance) {
      for (const entry of doc.provenance) {
        const key = entry.publicationKey || entry.pmid
        if (key) provenanceObj[key] = entry.researcherIds || []
      }
    }

    const publications = mergePublications((doc.publications || []).map(p => {
      const publicationTypes = normalizePublicationTypes(p.publicationTypes || p.pubTypes || p.pubtype)
      return withPublicationKey({
        ...
        missingRuns: Number(p.missingRuns) || 0,
        exclude: isPublicationExcluded({ ...p, publicationTypes })
      })
    }))

    const identityMigrationNeeded = publications.length !== (doc.publications || []).length ||
      (doc.publications || []).some(p =>
        p.publicationKey !== getPublicationKey(p) || (p.doi || '') !== normalizeDoi(p.doi)
      )
    ...
    return {
      ...
      publications,
      provenance: normalizePublicationProvenance(doc.publications || [], provenanceObj),
      meta: {
        cachePath: 'sanity:pubmedCache',
        identityMigrationNeeded,
        doiBackfillFailures: (doc.doiBackfillFailures || []).map((entry) => ({
          doi: entry?.doi || null,
...
function toSanityPublication(pub, idx = 0) {
  ...
  return {
    ...
    // Retention bookkeeping: when the record was last returned by discovery, and how many
    // consecutive clean runs have missed it. See lib/publicationRetention.js.
    lastSeenAt: normalized.lastSeenAt || null,
    missingRuns: Number(normalized.missingRuns) || 0,
...
export async function writeCache(payload) {
  ...
  try {
    ...
    const publicationsByKey = new Map((payload.publications || []).map((pub) => [getPublicationKey(pub), pub]))
    const provenanceArray = Object.entries(payload.provenance || {}).map(([publicationKey, ids]) => ({
    ...
    const doc = {
      ...
      // createOrReplace drops any field missing from this object, so the backfill
      // history has to be carried through the full write as well.
      doiBackfillFailures: toSanityBackfillFailures(payload.meta?.doiBackfillFailures),
      stats: {
 ...
 *
 * Pass `key` so the stored cacheKey stays in sync with the current researcher/query
 * configuration. Skipping this lets the app's computed key drift away from the stored
 * one, which in turn would make user-facing pages think the cache is stale and fall
 * back to slow (or failing) refresh paths during rendering.
 */
  ...
  try {
    ...
    if (updatedPublications.length > 0) {
      ...
      for (const pub of updatedPublications) {
        ...
        patch = patch.set({
          [`publications[_key=="${sanityPub._key}"]`]: sanityPub
        })
      }
    ...
    if (newPublications.length > 0 && Object.keys(provenance).length > 0) {
      const newProvenanceEntries = newPublications
        .map((pub) => ({ pub, publicationKey: getPublicationKey(pub) }))
        .filter(({ publicationKey }) => publicationKey && provenance[publicationKey])
        .map(({ pub, publicationKey }) => ({
          _key: toSanityPublicationKey({ publicationKey }),
          publicationKey,
          pmid: pub.pmid || null,
          researcherIds: Array.isArray(provenance[publicationKey])
            ? provenance[publicationKey]
            : Array.from(provenance[publicationKey] || []),
        }))
    ...
    if (typeof key === 'string' && key.length > 0) {
      finalSet.cacheKey = key
    }
    patch = patch.set(finalSet)

    // Commit all changes in one operation
    await patch.commit()

    const counts = await noCdnClient.fetch(
      `*[_type == $type && _id == $id][0]{
        "totalPublications": count(publications),
        "totalWithSummary": count(publications[defined(laySummary) && laySummary != ""])
      }`,
      { type: CACHE_DOC_TYPE, id: CACHE_DOC_ID }
    )

    const statsPatch = {
      'stats.totalPublications': counts?.totalPublications || 0,
      'stats.totalWithSummary': counts?.totalWithSummary || 0,
    }
    if (meta?.summaries?.model) {
      statsPatch['stats.lastSummaryModel'] = meta.summaries.model
    }

    // Update stats in a separate patch using exact totals from the saved document.
    await writeClient
      .patch(CACHE_DOC_ID)
      .setIfMissing({ stats: {} })
      .set(statsPatch)
...
export async function getExistingSummaries() {
  try {
    const doc = await noCdnClient.fetch(
      `*[_type == $type && _id == $id][0].publications[]{ publicationKey, pmid, doi, title, abstract, abstractContentType, abstractSource, laySummary, topics, studyDesign, methodologicalFocus, publicationTypes, exclude }`,
      { type: CACHE_DOC_TYPE, id: CACHE_DOC_ID }
    )
    ...
    for (const pub of doc) {
      ...
      if (publicationKey) {
        const publicationTypes = normalizePublicationTypes(pub.publicationTypes)
        map.set(publicationKey, {
          abstract: pub.abstract || null,
          abstractContentType: pub.abstractContentType || null,
          abstractSource: pub.abstractSource || null,
