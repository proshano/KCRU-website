...
// `item` is a publication feed entry from selectFeedPublications().
export function buildSocialPostRecord({
  ...
  const text = buildXPostText({ intro, title: publication.title, link: identity.link })
  ...
  return {
    ...
    teamMembers: Array.from(new Set((teamMembers || []).map(cleanString).filter(Boolean))),
    proposedText: text,
    text,
    status,
    createdAt: toIso(now),
...
export async function dispatchSocialPostNotifications({
  ...
  const due = selectSocialPostsToNotify(records, { now })
  const normalizedRecipients = Array.from(new Set((recipients || [])
    .map((email) => cleanString(email).toLowerCase())
    .filter(Boolean)))
  const summary = {
    due: due.length,
    newPosts: due.filter((post) => post.notificationKind === 'new').length,
    reminders: due.filter((post) => post.notificationKind === 'reminder').length,
    recipientCount: normalizedRecipients.length,
    postIds: due.map((post) => post._id),
  }
  if (!due.length) {
    return { ok: true, skipped: true, dryRun, reason: 'No social media post approval email is due.', ...summary }
  }
  ...
  const email = buildSocialPostApprovalEmail({ posts: due, portalUrl })
  ...
  if (delivery?.skipped) {
    throw new Error(`Social media post approval email was not sent: ${delivery.reason || 'email provider unavailable'}.`)
  }
