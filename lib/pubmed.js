import { normalizePublicationTypes } from './publicationExclusions.js'
import { normalizeDoi } from './publicationIdentity.js'
...
const DEFAULT_PUBMED_SEARCH_SORT = 'pub+date'
const PUBMED_SEARCH_SORT = process.env.PUBMED_SEARCH_SORT || DEFAULT_PUBMED_SEARCH_SORT
...
const SPACE_CODEPOINTS = new Set([
  0x00a0, // nbsp
  0x2009, // thin space
...
function decodeHtmlEntities(value) {
  ...
  return text.replace(/&(#x[0-9a-fA-F]+|#\d+|[A-Za-z]+);/g, (match, entity) => {
    ...
    if (SPACE_CODEPOINTS.has(codePoint)) return ' '
    try {
...
function cleanPubmedText(value) {
  const stripped = stripXmlTags(value)
  return decodeHtmlEntities(stripped)
}

function extractXmlValue(xml, tagName) {
  const match = String(xml || '').match(new RegExp(`<${tagName}(?:\\s[^>]*)?>([\\s\\S]*?)<\\/${tagName}>`, 'i'))
  return match ? cleanPubmedText(match[1]).replace(/\s+/g, ' ').trim() : ''
}

function parsePubmedAttributionAuthors(articleXml) {
  // Researcher queries include collaborators indexed in InvestigatorList, even when
  // they are absent from the displayed byline. Keep that evidence for attribution.
  const authorMatches = String(articleXml || '').match(/<(Author|Investigator)(?:\s[^>]*)?>[\s\S]*?<\/\1>/gi) || []
  return authorMatches.map((authorXml) => {
    const given = extractXmlValue(authorXml, 'ForeName') || extractXmlValue(authorXml, 'Initials')
    const family = extractXmlValue(authorXml, 'LastName')
    const displayName = [given, family].filter(Boolean).join(' ')
    const orcidMatch = authorXml.match(/<Identifier[^>]*Source=["']ORCID["'][^>]*>([\s\S]*?)<\/Identifier>/i)
    const affiliations = (authorXml.match(/<Affiliation(?:\s[^>]*)?>[\s\S]*?<\/Affiliation>/gi) || [])
      .map((value) => cleanPubmedText(value).replace(/\s+/g, ' ').trim())
      .filter(Boolean)
    return {
      given,
      family,
      displayName,
      orcid: orcidMatch ? cleanPubmedText(orcidMatch[1]).trim() : null,
      affiliations,
      ...(/^<Investigator\b/i.test(authorXml) ? { role: 'investigator' } : {}),
    }
  }).filter((author) => author.given && author.family)
}

function parsePubmedArticleDetails(articleXml) {
  const pmidMatch = String(articleXml || '').match(/<PMID[^>]*>(\d+)<\/PMID>/)
  if (!pmidMatch) return null
  const abstractParts = String(articleXml || '').match(/<AbstractText[^>]*>[\s\S]*?<\/AbstractText>/g) || []
  const abstract = abstractParts
    .map((part) => cleanPubmedText(part))
    .join(' ')
    .replace(/\s+/g, ' ')
    .trim()
  return {
    pmid: pmidMatch[1],
    abstract,
    attributionAuthors: parsePubmedAttributionAuthors(articleXml),
  }
}
...
function selectBestCandidate(candidates) {
  ...
  return candidates.reduce((best, candidate) => {
    ...
    const bestRank = PRECISION_RANK[best.precision] || 0
    const candidateRank = PRECISION_RANK[candidate.precision] || 0
    if (candidateRank > bestRank) return candidate
    if (candidateRank < bestRank) return best
    return candidate.startMs < best.startMs ? candidate : best
  }, null)
}
...
function parsePubDate(item = {}) {
  ...
  const bestPublication = selectBestCandidate(publicationCandidates)
  const bestSort = selectBestCandidate(sortCandidates)
  let best = bestPublication || bestSort
  if (bestPublication && bestSort) {
    const pubRank = PRECISION_RANK[bestPublication.precision] || 0
    const sortRank = PRECISION_RANK[bestSort.precision] || 0
    if (sortRank > pubRank) best = bestSort
  }
...
export async function searchPubMed(query, maxResults = 200) {
  const { pmids } = await searchPubMedWithTotal(query, maxResults)
  return pmids
}

/**
 * Search PubMed and return PMIDs alongside the total number of matches, so callers can
 * tell whether `maxResults` truncated the result set.
 * @param {string} query - PubMed search query
 * @param {number} maxResults - Maximum number of results
 * @returns {Promise<{pmids: string[], total: number}>}
 */
export async function searchPubMedWithTotal(query, maxResults = 200) {
  const url = `${BASE_URL}/esearch.fcgi?db=pubmed${apiKeyParam}&term=${encodeURIComponent(query)}&retmode=json&retmax=${maxResults}&sort=${encodeURIComponent(PUBMED_SEARCH_SORT)}`

  const data = await fetchJsonWithRetry(url)
  const pmids = data.esearchresult?.idlist || []
  const total = Number(data.esearchresult?.count)

  return { pmids, total: Number.isFinite(total) ? total : pmids.length }
}
    ...
    try {
      ...
      const publications = Object.values(data.result || {})
        ...
        .map(item => ({
          ...
          pmid: item.uid,
          title: cleanPubmedText(item.title || 'No title'),
          authors: (item.authors || []).map(a => a.name).filter(Boolean),
          journal: cleanPubmedText(item.source || ''),
          volume: item.volume || '',
          issue: item.issue || '',
          pages: item.pages || '',
          doi: normalizeDoi(item.articleids?.find(id => id.idtype === 'doi')?.value) ||
            normalizeDoi(item.elocationid),
          publicationTypes: normalizePublicationTypes(item.pubtype),
 ...
 */
export async function fetchPubmedArticleDetails(pmids) {
  ...
  for (const chunk of chunks) {
    ...
    try {
      ...
      const articleMatches = xml.match(/<(PubmedArticle|PubmedBookArticle)>[\s\S]*?<\/\1>/g) || []
      
      for (const articleXml of articleMatches) {
        const parsed = parsePubmedArticleDetails(articleXml)
        if (!parsed) continue
        details.set(parsed.pmid, parsed)
        parsedArticles += 1
        if (parsed.abstract) abstractsWithText += 1
      }
...
export async function fetchAbstracts(pmids) {
  const details = await fetchPubmedArticleDetails(pmids)
  return new Map(Array.from(details.entries()).map(([pmid, value]) => [pmid, value.abstract]))
}
...
export async function fetchPublicationsWithAbstracts(pmids) {
  const [publications, articleDetails] = await Promise.all([
    fetchPublicationDetails(pmids),
    fetchPubmedArticleDetails(pmids)
  ])

  return publications.map(pub => {
    const details = articleDetails.get(pub.pmid) || {}
    ...
    return {
      ...
      attributionAuthors: details.attributionAuthors || [],
    }
  })
}
