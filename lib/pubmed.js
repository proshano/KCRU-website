...
const MONTHS_MAP = Object.fromEntries(MONTHS_SHORT.map((m, idx) => [m, idx + 1]))
...
function parsePubDate(item = {}) {
  const candidates = [item.sortpubdate, item.epubdate, item.pubdate].filter(Boolean)

  for (const raw of candidates) {
    // sortpubdate arrives as YYYY/MM/DD HH:MM
    const sortMatch = raw.match(/^(\d{4})\/(\d{2})\/(\d{2})/)
    if (sortMatch) {
      const [, y, m, d] = sortMatch
      const iso = `${y}-${m}-${d}T00:00:00Z`
      return {
        isoDate: iso,
        year: Number(y),
        month: Number(m),
        monthName: MONTHS_SHORT[Number(m) - 1] || ''
      }
    }

    // Formats like "2024 Nov 5" or "2024 Nov"
    const monthMatch = raw.match(/^(\d{4})\s+([A-Za-z]{3})(?:\s+(\d{1,2}))?/)
    if (monthMatch) {
      const [, yRaw, monthRaw, dayRaw] = monthMatch
      const monthNum = MONTHS_MAP[monthRaw] || 1
      const dayNum = dayRaw ? Number(dayRaw) : 1
      const iso = `${yRaw}-${String(monthNum).padStart(2, '0')}-${String(dayNum).padStart(2, '0')}T00:00:00Z`
      return {
        isoDate: iso,
        year: Number(yRaw),
        month: monthNum,
        monthName: MONTHS_SHORT[monthNum - 1] || ''
      }
    }

    // Year-only strings such as "2023"
    const yearOnly = raw.match(/^(\d{4})/)
    if (yearOnly) {
      const y = Number(yearOnly[1])
      return {
        isoDate: `${y}-01-01T00:00:00Z`,
        year: y,
        month: 1,
        monthName: MONTHS_SHORT[0]
      }
    }
  }
