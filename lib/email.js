...
async function sendWithResend({ apiKey, from, to, subject, text, html, replyTo, attachments, idempotencyKey, headers }) {
  const messageHeaders = Object.fromEntries(
    Object.entries(headers || {}).filter(([key, value]) => key && String(value || '').trim())
  )
...
export async function sendEmail({ to, subject, text, html, replyTo, attachments, idempotencyKey, headers }) {
