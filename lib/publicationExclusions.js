...
// Repositories can index an article's attachments as separate "Journal Article"
// records, including both the base DOI and a versioned DOI for the same file.
const SUPPLEMENTARY_TITLE_PATTERNS = [
  /^\s*additional\s+files?\s+\d+[a-z]?\b/i,
  /^\s*(?:electronic\s+)?(?:supplementary|supplemental|supporting)\s+(?:materials?|information|data|files?|tables?|figures?|appendix|appendices)(?:\s+[a-z]?\d+)?\s*(?::|[-–—]|\b(?:of|for|to)\b|$)/i,
]

export function isPublicationSupplementaryMaterial(publication) {
  const title = String(publication?.title || '')
  return SUPPLEMENTARY_TITLE_PATTERNS.some(pattern => pattern.test(title))
}
...
export function isPublicationExcludedByRule(publication) {
  return isPublicationCorrectionNotice(publication) ||
    isPublicationCorrespondence(publication) ||
    isPublicationSupplementaryMaterial(publication)
}
