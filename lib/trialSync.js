 ...
 * and generate plain-language summaries using LLM
 * 
 * This module orchestrates:
 * 1. Fetching detailed trial info from ClinicalTrials.gov API
 * 2. Generating plain-language summary for patients
 * 3. Generating simplified eligibility overview
 * 4. Mapping data to Sanity schema structure
 */

import { getTrialByNCTId } from './clinicaltrials'
import { generateTrialSummary, generateEligibilityOverview } from './summaries'
...
function transformToSanitySchema(rawData) {
  ...
  return {
    ...
    ageRange: {
      minimum: rawData.eligibility?.minimumAge || null,
      maximum: rawData.eligibility?.maximumAge || null
    },
    sex: SEX_MAPPING[rawData.eligibility?.sex] || 'all',
    ...
    _forEligibilityOverview: {
      inclusionCriteria: rawData.eligibility?.inclusionCriteria || [],
      exclusionCriteria: rawData.eligibility?.exclusionCriteria || [],
      minimumAge: rawData.eligibility?.minimumAge,
      maximumAge: rawData.eligibility?.maximumAge
    }
  }
}
 ...
 * @param {boolean} options.generateSummary - Generate lay summary (default: true)
 * @param {boolean} options.generateEligibilityOverview - Generate eligibility overview (default: true)
...
export async function syncTrialData(nctId, options = {}) {
  const {
    ...
    generateEligibilityOverview: genEligibility = true
  } = options
  ...
  // Generate lay summary for patients
  let laySummary = null
  if (generateSummary && trialData._forSummaryGeneration) {
    console.log(`[trialSync] Generating lay summary for ${nctId}...`)
    try {
  ...
  // Generate simplified eligibility overview
  let eligibilityOverview = null
  if (genEligibility && trialData._forEligibilityOverview) {
    console.log(`[trialSync] Generating eligibility overview for ${nctId}...`)
    try {
      eligibilityOverview = await generateEligibilityOverview(trialData._forEligibilityOverview, {
        debug: process.env.LLM_DEBUG === 'true'
      })
    } catch (error) {
      console.error(`[trialSync] Error generating eligibility overview for ${nctId}:`, error.message)
    }
  }

  // Clean up internal fields
  delete trialData._forSummaryGeneration
  delete trialData._forEligibilityOverview
