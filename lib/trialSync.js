...
function titleIncludesAcronym(title, acronym) {
  if (!title || !acronym) return false
  const normalizedTitle = normalizeForCompare(title)
  const normalizedAcronym = normalizeForCompare(acronym)
  if (!normalizedAcronym) return false
  return normalizedTitle.includes(normalizedAcronym)
}

function looksLikeAcronym(value) {
  const cleaned = cleanAcronym(value)
...
function buildDisplayTitle({ briefTitle, officialTitle, acronym }) {
  const baseTitle = String(briefTitle || officialTitle || '').trim()
  const cleanedAcronym = cleanAcronym(acronym)
  const inferredAcronym =
    cleanedAcronym ||
    extractAcronymFromTitle(briefTitle) ||
    extractAcronymFromTitle(officialTitle)
  if (!baseTitle) {
    return { displayTitle: '', acronym: inferredAcronym }
  }
  if (!inferredAcronym || titleIncludesAcronym(baseTitle, inferredAcronym)) {
    return { displayTitle: baseTitle, acronym: inferredAcronym }
  }
  return { displayTitle: `${baseTitle} (${inferredAcronym})`, acronym: inferredAcronym }
}

function parseCtGovAgeToYears(value) {
  const cleaned = String(value || '').trim()
  if (!cleaned || /^n\/?a$/i.test(cleaned)) return null

  const match = cleaned.match(/^(\d+(?:\.\d+)?)\s*(year|years|month|months|week|weeks|day|days)$/i)
  ...
  if (unit.startsWith('month')) return Number((amount / 12).toFixed(1))
  if (unit.startsWith('week')) return Number((amount / 52).toFixed(1))
  if (unit.startsWith('day')) return Number((amount / 365).toFixed(1))
  return null
}

function mapCtGovSex(value) {
...
function buildPrescreenDefaults(rawData) {
  return {
    screeningSummary: '',
    sexAllowed: mapCtGovSex(rawData.eligibility?.sex),
    minimumAgeYears: parseCtGovAgeToYears(rawData.eligibility?.minimumAge),
    maximumAgeYears: parseCtGovAgeToYears(rawData.eligibility?.maximumAge),
    populationTags: [],
    ckdStages: [],
...
function transformToSanitySchema(rawData) {
  ...
  const { displayTitle, acronym } = buildDisplayTitle({
  ...
  return {
    ...
    prescreen: buildPrescreenDefaults(rawData),
