...
import { applyPublicationClassificationOverlay } from '@/lib/publicationClassificationOverlay'
...
function mergeClassifications(publications, classMap) {
  ...
  return publications.map((pub) => {
    ...
    return applyPublicationClassificationOverlay(pub, merged)
  })
}
...
function buildHighlight(pub, summaryMaxChars, tagsPerHighlight, index) {
  ...
  const key = getPublicationKey(pub) || `pub-${index}`
...
export async function getPublicationSeoSnapshot(options = {}) {
  ...
  const filtered = merged.filter((pub) => !isPublicationExcluded(pub))

  const highlights = filtered
    .slice(0, highlightLimit)
    .map((pub, index) => buildHighlight(pub, summaryMaxChars, tagsPerHighlight, index))
    .filter(Boolean)
