...
// Returns { text, generatedBy }. The text is the generated body plus the link:
// `link` when given (an investigator's profile anchored to the paper), else the
// paper link. When the model is unavailable, returns nothing, or fails the checks
...
export async function composeSocialPostDraft({
  ...
  const link = cleanString(linkOverride) || cleanString(post.link)
