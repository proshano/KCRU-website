...
function normalizeAssumptions(value) {
  ...
  // Assumptions are generated here and round-tripped through the client, so the list arrives back
  // caller-controlled and lands in the LLM prompt. Only a handful are ever produced; bound it.
  return uniqueList(value.map((item) => normalizeString(item).slice(0, MAX_ASSUMPTION_LENGTH))).slice(
    0,
    MAX_ASSUMPTIONS
  )
}
