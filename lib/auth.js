...
import { resolveCoordinatorDomains } from '@/lib/coordinatorDomains'
...
async function fetchAccessConfig() {
  ...
  return {
    ...
    domains: resolveCoordinatorDomains(settings?.coordinatorDomains)
  }
}
...
export async function getAuthAccess(email) {
  ...
  const domainOk = config.domains.some((domain) => normalized.endsWith(`@${domain}`))
