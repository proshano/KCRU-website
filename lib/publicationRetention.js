...
import { isPublicationSupplementaryMaterial } from './publicationExclusions.js'
 ...
 * @param {boolean} requireAttribution Remove records that have no valid researcher link.
...
export function retainPublications({
  ...
  const publications = []
  for (const pub of fetchedPublications) {
    const publicationKey = getPublicationKey(pub)
    if (isPublicationSupplementaryMaterial(pub)) {
      delete provenance[publicationKey]
      continue
    }
    const researcherIds = toIdArray(fetchedProvenance[publicationKey])
    if (requireAttribution && researcherIds.length === 0) continue
    if (publicationKey) fetchedKeys.add(publicationKey)
    publications.push({ ...pub, lastSeenAt: seenAt, missingRuns: 0 })
  }
  ...
  for (const pub of cachedPublications) {
    ...
    if (isPublicationSupplementaryMaterial(pub) || !isRetainable(pub)) {
    ...
    if (requireAttribution && researcherIds.length === 0) {
      removed.push({ publication: pub, publicationKey, reason: 'no-valid-attribution' })
      continue
    }
