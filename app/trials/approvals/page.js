...
export default async function TrialsApprovalsPage() {
  const session = await getServerSession(authOptions)
  const access = session?.user?.access

  if (!session?.user?.email) {
    redirect('/login?callbackUrl=/trials/approvals')
  }
  ...
  const content = !access?.approvals ? (
    <div className="mx-auto max-w-2xl px-6 py-16">
      ...
      <div className="mt-6">
        <AuthButtons signInCallbackUrl="/trials/approvals" signOutCallbackUrl="/login" />
      </div>
    </div>
  ) : (
    <Suspense
