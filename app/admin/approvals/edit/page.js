...
import ApprovalEditClient from '@/app/trials/approvals/edit/ApprovalEditClient'
...
export default async function AdminApprovalEditPage() {
  const session = await getServerSession(authOptions)
  const access = session?.user?.access

  if (!session?.user?.email) {
    redirect('/login?callbackUrl=/admin/approvals/edit')
  }

  const content = !access?.approvals ? (
    <div className="mx-auto max-w-2xl px-6 py-16">
      ...
      <div className="mt-6">
        <AuthButtons signInCallbackUrl="/admin/approvals/edit" signOutCallbackUrl="/login" />
      </div>
    </div>
  ) : (
    <Suspense
